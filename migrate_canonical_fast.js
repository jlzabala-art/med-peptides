...
async function migrate() {
  ...
  await Promise.all(snapshot.docs.map(async (doc) => {
    const data = doc.data();
    if (!data.name || data._isCanonical) return;
    ...
    // Group them
    // Using a simple array push isn't strictly thread safe in some languages, but in JS event loop it's fine
    // However to be absolutely safe:
    return { canonicalName, doc, data, variants };
  })).then(results => {
    for (const res of results) {
      if (!res) continue;
      if (!groups.has(res.canonicalName)) groups.set(res.canonicalName, []);
      groups.get(res.canonicalName).push({
        id: res.doc.id,
        ref: res.doc.ref,
        data: res.data,
