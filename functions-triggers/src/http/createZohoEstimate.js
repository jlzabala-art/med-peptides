...
exports.createZohoEstimate = onCall(async (request) => {
  // Ensure the user is authenticated (unless we are bypassing it for dev)
  // if (!request.auth) {
  //   throw new HttpsError('unauthenticated', 'User must be logged in.');
  // }

  const { queueItemId, customerName, products } = request.data;

  if (!queueItemId) {
    throw new HttpsError('invalid-argument', 'queueItemId is required');
  }
  ...
  try {
    ...
    throw new HttpsError('internal', 'Failed to create Zoho Estimate');
  }
});
