...
/**
 * Hook estandarizado para obtener pacientes utilizando React Query y Repositorios.
...
export default function usePatients(filters = {}, options = {}) {
  ...
  const queryKey = ['patients', filters, pageSize, orderByDesc];
  const cacheKey = `__rg_patients_cache_${filterStr}_${pageSize}_${orderByDesc}`;
  ...
  const {
    ...
    queryFn: ({ pageParam = null }) => 
      PatientRepository.getPatientsPage({ filters, pageSize, pageParam, orderByDesc }),
    ...
    initialData: () => {
      if (typeof window !== 'undefined') {
        try {
          ...
          logger.warn('[usePatients] Failed to read cached initial data from localStorage:', e);
        }
      }
      return undefined;
    }
  ...
  useEffect(() => {
    ...
    if (infiniteData?.pages?.[0] && !infiniteData.pages[0].fromCache && typeof window !== 'undefined') {
      try {
        ...
        logger.warn('[usePatients] Failed to persist page cache in localStorage:', e);
      }
    }

    if (!realtime) return;
    
    const q = PatientRepository.buildQuery(filters, pageSize, null, orderByDesc);
    const unsub = onSnapshot(q, (snap) => {
      ...
      logger.error('[usePatients] Error in realtime sync:', err);
    });
