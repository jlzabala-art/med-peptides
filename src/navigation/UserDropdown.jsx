...
function MenuItem({ label, path, onClose, danger = false }) {
  ...
  return (
    <Link
      ...
      onClick={onClose}
    >
      <span className="dropdown-item-icon">{PATH_ICONS[path] || <User size={15} />}</span>
      {label}
    </Link>
  );
}
...
export default function UserDropdown({
  ...
  const menuRoleKey =
    activeRole === 'admin'
      ? 'admin'
      : ['clinic', 'doctor', 'wholesaler', 'compounding_pharmacy', 'supplier', 'staff'].includes(
            activeRole
          )
        ? 'professional'
        : 'guest';
  ...
  const items = rawItems.map((item) => {
    if (item.path === '/settings') {
      ...
      else if (activeRole === 'wholesaler' || activeRole === 'clinic' || activeRole === 'pharmacy')
        dynamicPath = '/wholesaler/settings';
    ...
    <>
      ...
      >
        ...
        <div className="dropdown-body" style={{ padding: '0.5rem' }}>
          ...
          {['admin', 'clinic', 'doctor', 'compounding_pharmacy', 'wholesaler', 'supplier'].includes(
            baseRole
          ) &&
            (() => {
              let options = [];
