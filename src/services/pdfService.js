...
const addRunningFooter = (doc) => {
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text("CONFIDENTIAL CLINICAL RECORD - Generated by Atlas Health Engine. Intended for research use.", 14, pageHeight - 15);
    doc.setTextColor(100, 116, 139);
...
const setupDosageGuideHeader = (doc, protocol, formData) => {
  ...
  doc.text(`Issuing Authority: Atlas Health Clinical Pharmacist Office`, pageWidth - 14, 32, { align: 'right' });
  ...
  const clinician  = protocol.protocol_author_name || formData?.clinician || 'Atlas Health Clinical Team';
