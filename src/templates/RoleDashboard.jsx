...
import AdminPrescriptionsTab from '../components/admin/prescriptions/AdminPrescriptionsTab';
...
function WorkplaceLoadingFallback() {
  return (
    ...
    >
      <div
        className="workplace-pulse-loader"
        style={{
          width: '44px',
          height: '44px',
          borderRadius: '50%',
...
export default function RoleDashboard({ onBack }) {
  ...
  const TAB_REGISTRY = {
    ...
    products: { component: AdminProductsTab, label: 'Catalog & Items', icon: PackageSearch },
    ...
    protocols: { component: AdminProtocolsTab, label: 'Global Protocols', icon: ClipboardList },
    ...
    prescriptions: {
      component: DoctorPrescriptionsTab,
      label: 'Prescriptions',
      icon: ClipboardList,
    },
    admin_prescriptions: {
      component: AdminPrescriptionsTab,
      label: 'Global Prescriptions',
      icon: ClipboardList,
    },
    prescription_intake: {
      component: AdminPrescriptionIntakeTab,
      label: 'Prescription Intake',
      icon: FilePlus,
    },
    views: { component: AdminViewsConfigTab, label: 'Simulate Roles', icon: Layers },
  };
  ...
  async function fetchViewConfig(currentRoleKey) {
    try {
      ...
      if (docSnap.exists()) {
        ...
        // Fallbacks for known missing configs (e.g. after a backup restore)
        let fallbackTabs = {};
        if (searchKey === 'medical_director') {
          fallbackTabs = {
            prescriptions: { order: 1 },
            patients: { order: 2 },
            doctor_protocols: { order: 3 },
            prescription_intake: { order: 4 },
          };
        }
          ...
          {enabledTabsKeys.length === 0 ? (
            ...
            >
              ...
              <p style={{ color: 'var(--text-muted)' }}>
                Your profile currently has no active modules assigned. Please contact the
                administrator.
              </p>
            </div>
          ) : (
            (() => {
              ...
              return <Component {...tabConfig} userId={user.uid} doctorId={user.uid} />;
            })()
          )}
        </React.Suspense>
      </div>
