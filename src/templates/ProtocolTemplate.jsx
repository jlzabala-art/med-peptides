...
const CategoryProtocolNavigator = memo(function CategoryProtocolNavigator({
  ...
  useEffect(() => {
    ...
    const fetchObjective = primaryGoal ? getTemplatesByObjective(primaryGoal) : Promise.resolve([]);
    ...
    fetchObjective
      .then((list) => {
        ...
        // Fallback to prefix
        const prefix = currentSlug.split('_')[0];
        return getTemplatesByPrefix(prefix).then((fallbackList) => {
          if (!active) return;
          processList(fallbackList);
        });
      })
      .catch((err) => {
        console.warn('[CategoryProtocolNavigator] Fetch siblings failed:', err);
  ...
  const getSlug = (p) => p.protocol_slug || p.protocol_id || p.id;
  ...
  return (
    <>
      ...
      {/* ── Mobile navigator bar (inline responsive) ── */}
      <div
      ...
      >
        ...
        <div style={{ display: 'flex', gap: '0.5rem', width: '100%' }}>
          ...
          >
            <ChevronLeft size={15} strokeWidth={2.5} style={{ color: 'rgba(255,255,255,0.6)', flexShrink: 0 }} />
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', overflow: 'hidden', width: '100%' }}>
              <span style={{ fontSize: '0.55rem', color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 700 }}>Anterior</span>
              <span style={{ fontSize: '0.72rem', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap', width: '100%' }}>
                Anterior
              </span>
            </div>
          </button>
          ...
          >
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', overflow: 'hidden', width: '100%' }}>
              <span style={{ fontSize: '0.55rem', color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 700 }}>Siguiente</span>
              <span style={{ fontSize: '0.72rem', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap', width: '100%' }}>
                Siguiente
              </span>
            </div>
            <ChevronRight size={15} strokeWidth={2.5} style={{ color: 'rgba(255,255,255,0.6)', flexShrink: 0 }} />
          </button>
        </div>
      </div>
    </>
...
function SectionAccordion({ id, title, icon: Icon, defaultOpen = false, children, accentColor = 'var(--color-primary)' }) {
  const [open, setOpen] = useState(false);
...
{/* status badge removed from sticky header */}
      ...
      <div className="proto-detail__body">
        <style>{`
          /* 3-column layout matching ProductDetail */
          .pt-grid {
            display: grid;
          ...
          @media (max-width: 1200px) {
            .pt-grid { grid-template-columns: 220px 1fr 300px; gap: 2rem; }
          }
          @media (max-width: 1024px) {
            .pt-grid { grid-template-columns: 220px 1fr; gap: 2rem; }
