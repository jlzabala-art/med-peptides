  ...
  return (
    <div className="my-4 rounded-xl border border-[var(--rp-theme-border)] bg-[var(--rp-theme-surface)] overflow-hidden shadow-sm">
      <div className="bg-[var(--rp-theme-surface-hover)] p-3 border-b border-[var(--rp-theme-border)] flex items-center justify-between">
        <div>
          <h4 className="text-sm font-semibold text-[var(--rp-theme-text)]">Price Import Analysis</h4>
        ...
        <div className="flex items-center gap-2">
          <select 
            ...
            className="text-xs p-1 rounded bg-[var(--rp-theme-background)] border border-[var(--rp-theme-border)] text-[var(--rp-theme-text)]"
          >
        ...
        <table className="w-full text-left text-sm whitespace-nowrap">
          <thead className="bg-[var(--rp-theme-surface)] sticky top-0 border-b border-[var(--rp-theme-border)]">
            <tr>
              ...
              <th className="p-2 text-[var(--rp-theme-text-muted)] font-medium text-right">File Price</th>
              <th className="p-2 text-[var(--rp-theme-text-muted)] font-medium text-right">DB Price ({targetTier})</th>
            </tr>
          </thead>
            ...
            {data.comparison.map((item, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="p-2 text-right font-medium text-blue-400">
                    ${item.filePrice?.toFixed(2) || '---'}
                  </td>
                  <td className="p-2 text-right">
                    {item.matched ? (
                      <div className="flex flex-col items-end">
                        <span className="text-[var(--rp-theme-text)]">${item.dbPrice?.toFixed(2) || '---'}</span>
                        {Math.abs(diff) > 0.1 && (
