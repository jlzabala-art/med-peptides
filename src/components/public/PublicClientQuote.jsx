    ...
    <div style={{ minHeight: '100vh', background: '#f0fdf4', padding: '3rem 1rem' }}>
      <div style={{ maxWidth: '800px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '2rem', justifyContent: 'center', color: '#166534' }}>
          <ShieldCheck size={20} />
          <span style={{ fontSize: '0.9rem', fontWeight: 600, letterSpacing: '1px', textTransform: 'uppercase' }}>Client Quote Portal</span>
        </div>
        ...
        <Card style={{ padding: '0', overflow: 'hidden', boxShadow: '0 10px 25px rgba(0,0,0,0.05)', border: '1px solid #bbf7d0' }}>
          <div style={{ padding: '2rem', background: 'white', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <h1 style={{ margin: '0 0 0.5rem', fontSize: '1.5rem', color: '#0f172a' }}>Quote #{id.slice(0,6).toUpperCase()}</h1>
              <p style={{ margin: 0, color: '#64748b' }}>Prepared for: <strong>{rfq.clientName}</strong></p>
            </div>
            {status === 'APPROVED' && <span style={{ padding: '0.5rem 1rem', background: '#dcfce7', color: '#166534', borderRadius: '20px', fontWeight: 700, display: 'flex', gap: '0.5rem', alignItems: 'center' }}><CheckCircle size={18}/> APPROVED</span>}
            {status === 'REJECTED' && <span style={{ padding: '0.5rem 1rem', background: '#fee2e2', color: '#b91c1c', borderRadius: '20px', fontWeight: 700, display: 'flex', gap: '0.5rem', alignItems: 'center' }}><XCircle size={18}/> REJECTED</span>}
          </div>
          ...
          <div style={{ padding: '2rem', background: '#fafafa' }}>
            <table className="gcp-table" style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.95rem' }}>
              <thead>
                <tr>
                  <th style={{ textAlign: 'left', padding: '1rem', background: '#f8fafc' }}>Item</th>
                  <th style={{ textAlign: 'center', padding: '1rem', background: '#f8fafc' }}>Qty</th>
                  <th style={{ textAlign: 'right', padding: '1rem', background: '#f8fafc' }}>Unit Price</th>
                  <th style={{ textAlign: 'right', padding: '1rem', background: '#f8fafc' }}>Total</th>
                </tr>
              </thead>
              <tbody>
            ...
            {(rfq.status === 'SHIPPED' || rfq.status === 'DELIVERED') && rfq.shippingData && (
              <div style={{ marginTop: '2rem', padding: '1.5rem', background: '#eff6ff', borderRadius: '8px', border: '1px solid #bfdbfe' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem', color: '#1d4ed8' }}>
                  <Truck size={24} />
                  <h3 style={{ margin: 0, fontSize: '1.2rem', fontWeight: 700 }}>Tracking Information</h3>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem', fontSize: '0.95rem' }}>
                  <div>
                    <strong style={{ color: '#1e3a8a', display: 'block' }}>Carrier</strong>
                    <span style={{ color: '#334155' }}>{rfq.shippingData.carrier}</span>
                  </div>
                  <div>
                    <strong style={{ color: '#1e3a8a', display: 'block' }}>Tracking (AWB)</strong>
                    <span style={{ color: '#3b82f6', fontWeight: 600, cursor: 'pointer', textDecoration: 'underline' }}>{rfq.shippingData.awb}</span>
                  </div>
                  <div>
                    <strong style={{ color: '#1e3a8a', display: 'block' }}>Est. Delivery</strong>
