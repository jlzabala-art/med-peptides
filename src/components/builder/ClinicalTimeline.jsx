    ...
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.6rem' }}>
        ...
        <span style={{
          ...
          color: isOptimized ? '#059669' : '#d97706',
        }}>
      ...
      {/* Bars */}
      {[{ label: 'Before', value: before, pct: beforePct, color: '#94a3b8' },
        { label: 'After',  value: after,  pct: afterPct,  color: isOptimized ? '#10b981' : '#f59e0b' }]
      ...
      <div style={{ textAlign: 'right', marginTop: '4px', fontSize: '0.65rem', color: 'var(--text-muted)', fontWeight: 700 }}>
        Net change:&nbsp;
        <span style={{ color: isOptimized ? '#059669' : '#d97706', fontWeight: 900 }}>
  ...
  const phases = useMemo(() => {
    ...
    timeline.forEach(weekData => {
      ...
      if (!currentPhase || currentPhase.name !== wPhaseName) {
        ...
        else if (pName.includes('maintenance')) pColor = '#10b981';
      ...
      {impactSummary.total > 0 && (
        <div style={{
          ...
          {impactSummary.decreaseCount > 0 && (
            <span style={{ display: 'flex', alignItems: 'center', gap: '3px', color: '#3b82f6', backgroundColor: '#dbeafe', padding: '2px 8px', borderRadius: '20px' }}>
              <TrendingDown size={10} /> {impactSummary.decreaseCount} decrease
            </span>
          )}
          {impactSummary.removedCount > 0 && (
            <span style={{ display: 'flex', alignItems: 'center', gap: '3px', color: '#ef4444', backgroundColor: '#fee2e2', padding: '2px 8px', borderRadius: '20px' }}>
                    ...
                    <div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                        <div style={{
                          ...
                          Week {w.week}
                          {hasSafetyIssue && <AlertTriangle size={14} color={isActive ? 'white' : '#ef4444'} />}
                        </div>
                        <div style={{
                          fontSize: '0.65rem',
                          fontWeight: 700,
