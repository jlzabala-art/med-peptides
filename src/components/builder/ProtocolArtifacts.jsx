  ...
  <div style={{ marginBottom: '3rem', borderBottom: '2px solid #e2e8f0', paddingBottom: '2rem' }}>
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
        ...
        <div style={{ fontSize: '1.5rem', fontWeight: 900, color: 'var(--primary)', letterSpacing: '-0.02em' }}>ReGen PEPT Clinical</div>
      </div>
      <div style={{ textAlign: 'right' }}>
        <div style={{ fontSize: '0.7rem', fontWeight: 900, color: '#64748b', textTransform: 'uppercase' }}>Document Type</div>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '2rem' }}>
      <div>
        ...
        <div style={{ display: 'flex', gap: '2rem' }}>
          <div>
            <div style={{ fontSize: '0.65rem', fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase' }}>Protocol ID</div>
            <div style={{ fontFamily: 'Roboto Mono, monospace', fontSize: '0.9rem', fontWeight: 700 }}>{id}</div>
          </div>
          <div>
            <div style={{ fontSize: '0.65rem', fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase' }}>Start Date</div>
...
export const PatientAdministrationGuide = ({ data }) => {
  ...
  return (
    <div className="artifact-preview" style={{ background: 'white', color: '#1e293b', minHeight: '842px', padding: '4rem', maxWidth: '800px', margin: '0 auto', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.1)', fontFamily: 'Inter, sans-serif' }}>
      <ArtifactHeader 
      ...
      <section style={{ marginBottom: '3rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem' }}>
          {[
            { label: 'Weight Regulation', icon: <CheckCircle2 size={16} color="#10b981" /> },
            { label: 'Metabolic Optimization', icon: <CheckCircle2 size={16} color="#10b981" /> },
            { label: 'Energy Stabilization', icon: <CheckCircle2 size={16} color="#10b981" /> }
          ].map((goal, i) => (
            <div key={i} style={{ padding: '1rem', background: '#f8fafc', borderRadius: '12px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              {goal.icon}
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', borderRadius: '12px', overflow: 'hidden', border: '1px solid #e2e8f0' }}>
          <thead>
            <tr style={{ background: '#f8fafc', textAlign: 'left' }}>
              <th style={{ padding: '1rem', fontSize: '0.75rem', fontWeight: 900, color: '#64748b' }}>WEEK</th>
              <th style={{ padding: '1rem', fontSize: '0.75rem', fontWeight: 900, color: '#64748b' }}>MEDICATION</th>
              <th style={{ padding: '1rem', fontSize: '0.75rem', fontWeight: 900, color: '#64748b' }}>DOSE</th>
              <th style={{ padding: '1rem', fontSize: '0.75rem', fontWeight: 900, color: '#64748b' }}>DAY</th>
            </tr>
          </thead>
          <tbody>
            {timeline.slice(0, 12).map((w, i) => {
              ...
              return meds.map((c, j) => (
                <tr key={`${i}-${j}`} style={{ borderTop: j === 0 ? '1px solid #e2e8f0' : 'none' }}>
                  ...
                  <td style={{ padding: '1rem', fontSize: '0.85rem', fontWeight: 800, color: '#64748b' }}>{(c.days || []).join(', ') || c.frequency}</td>
                </tr>
              ));
            })}
          </tbody>
          ...
          <div style={{ display: 'grid', gap: '1rem' }}>
            {[
              ...
              <div key={i} style={{ display: 'flex', gap: '1rem', alignItems: 'flex-start' }}>
                ...
                <div style={{ fontSize: '0.9rem', fontWeight: 600, color: '#475569' }}>{step}</div>
              </div>
            ))}
          </div>
          <div style={{ background: '#f8fafc', borderRadius: '16px', border: '1px solid #e2e8f0', padding: '1rem', textAlign: 'center' }}>
             <div style={{ fontSize: '0.65rem', fontWeight: 900, color: '#94a3b8', textTransform: 'uppercase', marginBottom: '1rem' }}>Site Rotation Recommended</div>
             <div style={{ height: '120px', background: 'white', borderRadius: '12px', border: '1px dashed #cbd5e1', display: 'flex', alignItems: 'center', justify: 'center', color: '#cbd5e1' }}>
                <TrendingUp size={48} />
             </div>
          </div>
        </div>
    ...
    <div className="artifact-preview" style={{ background: 'white', color: '#1e293b', minHeight: '842px', padding: '4rem', maxWidth: '800px', margin: '0 auto', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.1)', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1.5rem', marginBottom: '3rem' }}>
        <div style={{ padding: '1.5rem', background: '#f8fafc', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
          <div style={{ fontSize: '0.7rem', fontWeight: 900, color: '#64748b', textTransform: 'uppercase', marginBottom: '1rem' }}>Clinical Status</div>
          <div style={{ display: 'grid', gap: '0.75rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ fontSize: '0.85rem', fontWeight: 600 }}>Safety Status:</span>
              <span style={{ fontSize: '0.85rem', fontWeight: 900, color: validation.state === 'REJECTED' ? '#ef4444' : '#10b981' }}>
        ...
        <div style={{ padding: '1.5rem', background: '#f8fafc', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
          <div style={{ fontSize: '0.7rem', fontWeight: 900, color: '#64748b', textTransform: 'uppercase', marginBottom: '1rem' }}>Key Recommendations</div>
      ...
      <section style={{ marginBottom: '3rem' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', borderRadius: '12px', overflow: 'hidden', border: '1px solid #e2e8f0' }}>
          <thead>
            <tr style={{ background: '#f8fafc', textAlign: 'left' }}>
              <th style={{ padding: '1rem', fontSize: '0.75rem', fontWeight: 900, color: '#64748b' }}>WEEK</th>
              <th style={{ padding: '1rem', fontSize: '0.75rem', fontWeight: 900, color: '#64748b' }}>CHECK TYPE</th>
              <th style={{ padding: '1rem', fontSize: '0.75rem', fontWeight: 900, color: '#64748b' }}>PARAMETERS</th>
            </tr>
          </thead>
          <tbody>
            {monitoring.length > 0 ? (
              ...
              [
                ...
                <tr key={i} style={{ borderTop: '1px solid #e2e8f0' }}>
                  ...
                  <td style={{ padding: '1.25rem', fontSize: '0.85rem', fontWeight: 600, color: '#64748b' }}>{m.params}</td>
                </tr>
              ))
            )}
          </tbody>
      ...
      <section style={{ marginBottom: '3rem' }}>
        ...
        <div style={{ display: 'grid', gap: '1rem' }}>
          {(validation.details || [
            ...
            <div key={i} style={{ padding: '1.25rem', border: '1px solid #fee2e2', background: '#fef2f2', borderRadius: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
          ...
          {validation.details?.filter(d => d.status === 'FLAG_RAISED').length === 0 && (
             <div style={{ padding: '1.25rem', border: '1px solid #dcfce7', background: '#f0fdf4', borderRadius: '12px', color: '#166534', fontWeight: 700, fontSize: '0.9rem' }}>
      ...
      <section>
        ...
        <div style={{ height: '150px', border: '2px solid #e2e8f0', borderRadius: '12px', backgroundColor: '#f8fafc', padding: '1rem', fontSize: '0.9rem', color: '#64748b' }}>
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, backgroundColor: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justify: 'center', padding: '2rem' }}>
       <div style={{ width: '100%', maxWidth: '900px', height: '90vh', backgroundColor: '#f1f5f9', borderRadius: '24px', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          <div style={{ padding: '1.5rem 2rem', background: 'white', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
             <div>
                <div style={{ fontSize: '0.7rem', fontWeight: 900, color: '#64748b', textTransform: 'uppercase' }}>Artifact Preview</div>
                <div style={{ fontSize: '1.1rem', fontWeight: 900, color: 'var(--primary)' }}>
                   ...
                   {type === 'json' && 'Clinical Pathway Definition (JSON)'}
                ...
                <button 
                  ...
                  onClick={() => {
                    ...
                    if (type === 'json') {
                      ...
                      a.href = url;
                      a.download = `Clinical_Pathway_${data.protocol_id || 'EXPORT'}.json`;
                      a.click();
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '3rem 0' }}>
             ...
             {type === 'json' && (
                <div style={{ maxWidth: '800px', margin: '0 auto', background: '#1e293b', color: '#94a3b8', padding: '2rem', borderRadius: '16px', fontFamily: 'Roboto Mono, monospace', fontSize: '0.85rem', lineHeight: 1.6 }}>
             ...
             {type === 'ics' && (
                <div style={{ maxWidth: '800px', margin: '0 auto', background: 'white', padding: '2rem', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
                      ...
                      {timeline_event_list.slice(0, 50).map((evt, i) => (
                        <div key={i} style={{ padding: '1rem', border: '1px solid #e2e8f0', borderRadius: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                           <div>
                              ...
                              <div style={{ fontSize: '0.8rem', color: '#64748b' }}>{evt.desc}</div>
                           </div>
                           <div style={{ textAlign: 'right' }}>
                   ...
                   <div style={{ marginTop: '2rem', padding: '1.5rem', background: '#f8fafc', borderRadius: '12px', border: '1px solid #cbd5e1' }}>
                      <div style={{ fontSize: '0.7rem', fontWeight: 900, color: '#64748b', textTransform: 'uppercase', marginBottom: '1rem' }}>Raw ICS Header Preview</div>
                      <div style={{ fontSize: '0.75rem', fontFamily: 'monospace', color: '#334155', lineHeight: 1.5 }}>
                        BEGIN:VCALENDAR<br />
                        VERSION:2.0<br />
                        PRODID:-//ReGen PEPT//Clinical Calendar v5.2//EN<br />
