    ...
    <div className="animate-fade-in" style={{ maxWidth: '1200px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'minmax(300px, 1fr) 350px', gap: '2rem' }}>
      ...
      <div style={{ display: 'grid', gap: '2rem' }}>
        ...
        <div className="clinical-card-v5" style={{ padding: '2rem', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1.5rem' }}>
          <div>
            <div style={{ fontSize: '0.65rem', fontWeight: 900, color: '#64748b', textTransform: 'uppercase' }}>Protocol ID</div>
            <div style={{ fontFamily: 'Roboto Mono, monospace', fontSize: '1rem', fontWeight: 900, color: 'var(--primary)' }}>{protocolId}</div>
          </div>
          <div>
            <div style={{ fontSize: '0.65rem', fontWeight: 900, color: '#64748b', textTransform: 'uppercase' }}>Status</div>
            <div style={{ fontSize: '1rem', fontWeight: 900, color: isPaused ? '#f59e0b' : '#10b981', display: 'flex', alignItems: 'center', gap: '4px' }}>
               {isPaused ? <Pause size={14} /> : <Activity size={14} />} {isPaused ? 'PAUSED' : 'ACTIVE'}
            </div>
          </div>
          <div>
            <div style={{ fontSize: '0.65rem', fontWeight: 900, color: '#64748b', textTransform: 'uppercase' }}>Current Window</div>
            <div style={{ fontSize: '1rem', fontWeight: 900, color: 'var(--primary)' }}>Week {currentWeek} of {duration}</div>
          </div>
          <div>
            <div style={{ fontSize: '0.65rem', fontWeight: 900, color: '#64748b', textTransform: 'uppercase' }}>Completion State</div>
           ...
           <div className="clinical-card-v5" style={{ padding: '1.5rem' }}>
              ...
              <div style={{ display: 'grid', gap: '1rem' }}>
                 {timeline_event_list
                   ...
                   .slice(0, 2)
                   .map((act, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1rem', background: '#f8fafc', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                       ...
                       <div>
                          <div style={{ fontSize: '0.7rem', fontWeight: 900, color: '#64748b' }}>
                          ...
                          <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#94a3b8' }}>Dosing Level: {act.desc}</div>
                       </div>
                    </div>
                 ))}
                 {timeline_event_list.filter(e => e.category === 'Injections' && new Date(e.date) >= today).length === 0 && (
                   <div style={{ padding: '1rem', textAlign: 'center', color: '#94a3b8', fontSize: '0.8rem', fontWeight: 600 }}>No upcoming injections.</div>
                 )}
              </div>
           </div>
           <div className="clinical-card-v5" style={{ padding: '1.5rem' }}>
                   ...
                   .map((act, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1rem', background: '#f0f9ff', borderRadius: '12px', border: '1px solid #bae6fd' }}>
                       ...
                       <div>
                          ...
                          <div style={{ fontSize: '0.75rem', fontWeight: 700, color: '#64748b' }}>{act.desc}</div>
                       </div>
                    </div>
                 ))}
                 {timeline_event_list.filter(e => e.category === 'Monitoring' && new Date(e.date) >= today).length === 0 && (
                   <div style={{ padding: '1rem', textAlign: 'center', color: '#94a3b8', fontSize: '0.8rem', fontWeight: 600 }}>No upcoming monitoring.</div>
                 )}
              </div>
           </div>
        </div>
        ...
        <div className="clinical-card-v5" style={{ padding: '2rem' }}>
           <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              ...
              <div style={{ fontSize: '0.85rem', fontWeight: 800, color: '#64748b' }}>WEEK {currentWeek} PLAN</div>
           </div>
             ...
             {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'].map((day, i) => {
               ...
               return (
                 <div key={day} style={{ textAlign: 'center' }}>
                    <div style={{ fontSize: '0.65rem', fontWeight: 900, color: '#94a3b8', marginBottom: '0.75rem' }}>{day.substring(0, 3)}</div>
                    <div style={{ 
                       height: '80px', 
                       background: hasInjection ? 'var(--primary-soft)' : hasMonitoring ? '#fff7ed' : hasMilestone ? '#fef2f2' : '#f8fafc', 
                       borderRadius: '12px', 
                       border: '1px solid',
                       borderColor: hasInjection ? 'var(--primary-light)' : hasMonitoring ? '#ffedd5' : hasMilestone ? '#ef4444' : '#e2e8f0',
                       display: 'flex',
                       ...
                       {hasMilestone && <Zap size={16} color="#ef4444" />}
                       {(hasInjection || hasMonitoring || hasMilestone) && <div style={{ fontSize: '0.5rem', fontWeight: 900, color: hasInjection ? 'var(--primary)' : hasMonitoring ? '#c2410c' : '#ef4444' }}>EVENT</div>}
                       {!hasInjection && !hasMonitoring && !hasMilestone && <Clock size={16} color="#cbd5e1" />}
                    </div>
                 </div>
               );
             })}
      ...
      <div style={{ display: 'grid', gap: '2rem', height: 'fit-content' }}>
        ...
        {/* SAFETY STATUS */}
        <div className="clinical-card-v5" style={{ padding: '1.5rem', background: '#f0fdf4', border: '1px solid #bbf7d0' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
              <div style={{ width: '30px', height: '30px', borderRadius: '50%', background: '#10b981', display: 'flex', alignItems: 'center', justify: 'center', color: 'white' }}>
                 <ShieldAlert size={18} />
              </div>
           ...
           <p style={{ fontSize: '0.75rem', color: '#15803d', margin: 0, fontWeight: 600, lineHeight: 1.5 }}>
         ...
         {/* DOCUMENTS & ARTIFACTS */}
         <div className="clinical-card-v5" style={{ padding: '1.5rem' }}>
            <h3 style={{ fontSize: '0.8rem', fontWeight: 900, color: '#64748b', marginBottom: '1.25rem', textTransform: 'uppercase' }}>CLINICAL DOCUMENTS</h3>
            <div style={{ display: 'grid', gap: '0.75rem' }}>
               {manifest.map((doc, i) => (
                 <div key={i} style={{ display: 'grid', gap: '0.5rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem', background: '#f8fafc', borderRadius: '10px', border: '1px solid #e2e8f0' }}>
                       ...
                       <div style={{ display: 'flex', gap: '4px' }}>
                          <button onClick={() => setPreviewType(doc.type)} style={{ padding: '4px', color: '#64748b', background: 'white', border: '1px solid #cbd5e1', borderRadius: '4px', cursor: 'pointer' }} title="Preview"><Eye size={14} /></button>
                           <button 
                               ...
                               style={{ 
                                  padding: '4px', 
                                  color: (doc.type === 'dosage' && isGeneratingDosage) ? '#94a3b8' : '#64748b', 
                                  background: 'white', 
                    ...
                    {doc.type === 'ics' && (
                       <div style={{ marginTop: '0.5rem', background: 'white', border: '1px solid #e2e8f0', borderRadius: '10px', padding: '1rem' }}>
                          <div style={{ fontSize: '0.65rem', fontWeight: 900, color: '#64748b', textTransform: 'uppercase', marginBottom: '0.75rem', display: 'flex', justifyContent: 'space-between' }}>
                          ...
                          <div style={{ display: 'grid', gap: '0.5rem', maxHeight: '200px', overflowY: 'auto', paddingRight: '4px' }}>
                             {timeline_event_list.slice(0, 5).map((evt, j) => (
                                <div key={j} style={{ padding: '0.5rem', border: '1px solid #f1f5f9', background: '#f8fafc', borderRadius: '6px' }}>
                                   <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2px' }}>
                                      <span style={{ fontSize: '0.7rem', fontWeight: 850, color: 'var(--primary)' }}>{evt.title}</span>
                                      <span style={{ fontSize: '0.6rem', fontWeight: 900, color: '#94a3b8' }}>{new Date(evt.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
                                   </div>
                                   <div style={{ fontSize: '0.6rem', color: '#64748b', fontWeight: 600 }}>{evt.desc}</div>
                                </div>
                             ))}
                             <div 
        ...
        {/* CLINICAL NOTES */}
        <div className="clinical-card-v5" style={{ padding: '1.5rem' }}>
           <h3 style={{ fontSize: '0.8rem', fontWeight: 900, color: '#64748b', marginBottom: '1rem', textTransform: 'uppercase' }}>CLINICAL NOTES</h3>
           <textarea 
             value={notes}
             onChange={(e) => setNotes(e.target.value)}
             style={{ width: '100%', height: '100px', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '0.75rem', fontSize: '0.85rem', fontWeight: 600, color: '#475569', resize: 'none', background: '#f8fafc' }}
           />
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.75rem' }}>
           ...
           <button onClick={onReset} className="btn-secondary-v5" style={{ padding: '0.75rem', border: '1px solid #fee2e2', color: '#ef4444', display: 'flex', alignItems: 'center', gap: '8px', justifyContent: 'center', gridColumn: 'span 2' }}>
