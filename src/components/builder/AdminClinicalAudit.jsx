...
export default function AdminClinicalAudit({ protocolData, templates }) {
  ...
  const auditResults = {
    integrity: [
      ...
      { id: '2', check: 'Compound Linkage', status: 'PASS', details: 'All 3 peptides resolve to valid Firestore references.' },
  ...
  return (
    <div className="clinical-card-v5" style={{ marginTop: '4rem', padding: '2.5rem', border: '1px solid #1e293b', background: '#0f172a', color: '#f8fafc' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div>
            ...
            <p style={{ margin: 0, fontSize: '0.75rem', color: '#94a3b8' }}>Low-level data relationship and integrity diagnostics.</p>
          </div>
        </div>
        <button 
      ...
      <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '1rem' }}>
        <button 
          ...
          style={{ background: 'none', border: 'none', color: activeTab === 'integrity' ? '#38bdf8' : '#94a3b8', fontSize: '0.75rem', fontWeight: 900, cursor: 'pointer', textTransform: 'uppercase' }}
        >
          Integrity Checks
        </button>
        <button 
          onClick={() => setActiveTab('relationships')}
          style={{ background: 'none', border: 'none', color: activeTab === 'relationships' ? '#38bdf8' : '#94a3b8', fontSize: '0.75rem', fontWeight: 900, cursor: 'pointer', textTransform: 'uppercase' }}
        >
          Relationship Mapping
        </button>
        <button 
          onClick={() => setActiveTab('runtime')}
          style={{ background: 'none', border: 'none', color: activeTab === 'runtime' ? '#38bdf8' : '#94a3b8', fontSize: '0.75rem', fontWeight: 900, cursor: 'pointer', textTransform: 'uppercase' }}
        >
          Runtime Validation
        </button>
      </div>
            ...
            <div key={item.id} style={{ padding: '1rem', background: 'rgba(255,255,255,0.02)', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
               <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '0.7rem', color: '#64748b' }}>{item.details}</div>
                  </div>
               </div>
          ...
          {auditResults.relationships.map((rel, i) => (
            <div key={i} style={{ padding: '1rem', background: 'rgba(255,255,255,0.02)', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.05)' }}>
               <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                  ...
                  <span style={{ fontSize: '0.65rem', fontWeight: 900, color: '#64748b' }}>Correlation: {rel.weight}</span>
               </div>
               <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
        ...
        <div style={{ animation: 'fadeIn 0.3s ease-out' }}>
          {!testSuiteResults && (
            <div style={{ textAlign: 'center', padding: '3rem', background: 'rgba(255,255,255,0.02)', borderRadius: '12px', border: '1px dashed rgba(255,255,255,0.1)' }}>
              <Terminal size={32} color="#64748b" style={{ marginBottom: '1rem' }} />
              <h4 style={{ margin: '0 0 0.5rem 0' }}>Clinical Test Runner</h4>
              <p style={{ fontSize: '0.85rem', color: '#94a3b8', marginBottom: '1.5rem' }}>Execute full architectural simulation to verify selection logic and compound integrity.</p>
              <button 
            ...
            <div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem', marginBottom: '2rem' }}>
                <div style={{ padding: '1rem', background: 'rgba(30, 41, 59, 0.5)', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.1)', textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: '0.65rem', fontWeight: 900, color: '#64748b', textTransform: 'uppercase' }}>Pass Rate</div>
                </div>
                <div style={{ padding: '1rem', background: 'rgba(30, 41, 59, 0.5)', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.1)', textAlign: 'center' }}>
                  <div style={{ fontSize: '1.5rem', fontWeight: 950, color: '#f8fafc' }}>{testSuiteResults.stats.total}</div>
                  <div style={{ fontSize: '0.65rem', fontWeight: 900, color: '#64748b', textTransform: 'uppercase' }}>Total Cases</div>
                </div>
                <div style={{ padding: '1rem', background: 'rgba(30, 41, 59, 0.5)', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.1)', textAlign: 'center' }}>
                  <div style={{ fontSize: '1.5rem', fontWeight: 950, color: '#10b981' }}>{testSuiteResults.stats.passed}</div>
                  <div style={{ fontSize: '0.65rem', fontWeight: 900, color: '#64748b', textTransform: 'uppercase' }}>Passed</div>
                </div>
                <div style={{ padding: '1rem', background: 'rgba(30, 41, 59, 0.5)', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.1)', textAlign: 'center' }}>
                  <div style={{ fontSize: '1.5rem', fontWeight: 950, color: '#ef4444' }}>{testSuiteResults.stats.failed}</div>
                  <div style={{ fontSize: '0.65rem', fontWeight: 900, color: '#64748b', textTransform: 'uppercase' }}>Failed</div>
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gap: '0.75rem' }}>
                {testSuiteResults.results.map((res, i) => (
                  <div key={i} style={{ padding: '1rem', background: 'rgba(255,255,255,0.02)', borderRadius: '10px', border: `1px solid ${res.status === 'PASS' ? 'rgba(16, 185, 129, 0.1)' : 'rgba(239, 68, 68, 0.2)'}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                      <div style={{ color: res.status === 'PASS' ? '#10b981' : '#ef4444' }}>
                        {res.status === 'PASS' ? <CheckCircle2 size={16} /> : <AlertCircle size={16} />}
                      </div>
                      <div>
                        <div style={{ fontSize: '0.85rem', fontWeight: 800 }}>{res.name}</div>
                        <div style={{ fontSize: '0.7rem', color: res.status === 'PASS' ? '#64748b' : '#ef4444' }}>{res.details}</div>
                      </div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                       <div style={{ fontSize: '0.65rem', fontWeight: 900, color: res.status === 'PASS' ? '#10b981' : '#ef4444' }}>{res.status}</div>
                       <div style={{ fontSize: '0.6rem', color: '#64748b' }}>{(res.id)}</div>
                    </div>
                  </div>
                ))}
              </div>
