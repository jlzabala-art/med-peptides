...
export default function ProtocolDashboard({ protocolData, onRegenerate }) {
  ...
  const getStatusStyle = (s) => {
      switch(s) {
          ...
          case 'archived': return { bg: '#f1f5f9', color: 'var(--color-text-secondary)', border: 'var(--color-border)' };
          ...
          return (
            <div 
              ...
              style={{
                ...
                padding: '2rem',
                borderRadius: '24px',
                backgroundColor: isActive ? 'var(--color-bg-app)' : 'white',
                    ...
                    <div style={{ flex: 1 }}>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '1.5rem' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                            ...
                            <span style={{ fontSize: '1rem', fontWeight: 900, color: 'var(--text-main)', backgroundColor: 'var(--color-bg-app)', padding: '4px 10px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                                {med.dosage}
                            </span>
                          </div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
      ...
      <div className="anim-fade-in">
        ...
        <div style={{ padding: '1.5rem', backgroundColor: 'var(--color-bg-app)', borderRadius: '16px', border: `1px solid ${isReview ? '#fee2e2' : 'var(--border)'}`, marginBottom: '1.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
            <div>
              ...
              <div style={{ fontWeight: 900, fontSize: '2.5rem', color: isOptimal ? 'var(--color-success)' : (isReview ? 'var(--color-danger)' : '#f59e0b'), display: 'flex', alignItems: 'baseline', gap: '0.25rem' }}>
            ...
            <div style={{ 
              ...
              color: isOptimal ? 'var(--color-success)' : (isReview ? '#b91c1c' : '#b45309'),
              textTransform: 'uppercase',
              letterSpacing: '0.05em'
            }}>
              {vState.replace(/_/g, ' ')}
                ...
                <div key={domain} style={{ position: 'relative' }}>
                  ...
                  {hoveredDomain === domain && (
                    <div style={{
                      ...
                      <div style={{ position: 'absolute', top: '100%', left: '10px', border: '6px solid transparent', borderTopColor: 'var(--color-text-primary)' }} />
                    </div>
                  )}
                  ...
                  <div style={{ height: '6px', backgroundColor: 'rgba(0,0,0,0.05)', borderRadius: '10px', overflow: 'hidden' }}>
                    <div style={{ 
                      width: `${dScore}%`, 
                      height: '100%', 
                      backgroundColor: dScore >= 90 ? 'var(--color-success)' : (dScore >= 70 ? '#f59e0b' : 'var(--color-danger)'),
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {validation?.details && validation.details.length > 0 ? (
            validation.details.map((detail, idx) => {
              ...
              return (
                <div key={idx} style={{ 
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                      {isWarning ? <AlertTriangle size={18} color="var(--color-danger)" /> : <Info size={18} color="var(--primary)" />}
                      <span style={{ fontWeight: 700, fontSize: '0.95rem', color: isWarning ? '#991b1b' : 'var(--text-main)' }}>
                        {detail.section}
                      </span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                      <span style={{ fontSize: '0.75rem', fontWeight: 700, color: isWarning ? 'var(--color-danger)' : 'var(--text-muted)' }}>
                        VIEW DETAILS
                      </span>
                      ...
                      <div style={{ display: 'grid', gap: '1rem' }}>
                        ...
                        {detail.error && (
                          <div>
                            ...
                            <div style={{ fontSize: '0.9rem', color: 'var(--color-danger)', fontWeight: 600 }}>{detail.error}</div>
                          </div>
                        )}
                        {detail.fix && (
                          <div style={{ backgroundColor: 'rgba(16, 185, 129, 0.05)', padding: '0.75rem', borderRadius: '8px', border: '1px solid rgba(16, 185, 129, 0.2)' }}>
                            <div style={{ fontSize: '0.7rem', fontWeight: 800, color: 'var(--color-success)', textTransform: 'uppercase', marginBottom: '0.25rem' }}>Recommended Fix</div>
            ...
            <div style={{ padding: '1.25rem', backgroundColor: 'var(--color-success-bg)', border: '1px solid #bbf7d0', borderRadius: '12px', color: '#166534', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
        ...
        <div style={{ 
          ...
          {exportStatus && (
            <div style={{ 
                ...
                fontSize: '0.9rem',
                fontWeight: 600,
                backgroundColor: exportStatus.type === 'error' ? 'var(--color-danger-bg)' : (exportStatus.type === 'success' ? 'var(--color-success-bg)' : '#f0f9ff'),
