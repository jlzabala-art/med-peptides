...
// ── Main Tab ──────────────────────────────────────────────────────────────────
export default function AdminBulkOrdersTabClient({ initialData = [], isSubTab = false }) {
  const { bulkOrders: hookOrders, loading, hasMore, loadMore, fetchBulkOrders: refreshOrders, totalCount } = useBulkOrders({ pageSize: 50 });
  const rawOrders = hookOrders.length > 0 ? hookOrders : initialData;
