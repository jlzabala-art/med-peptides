...
const STATUS_COLORS = {
  active:  { bg: '#d1fae5', text: '#065f46', dot: 'var(--color-success)' },
...
function KpiCard({ label, value, color, icon, onClick }) {
  return (
    <div
      ...
      onMouseEnter={e => { if (onClick) { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = `0 8px 24px ${color}22`; e.currentTarget.style.borderColor = `${color}77`; } }}
      onMouseLeave={e => { e.currentTarget.style.transform = ''; e.currentTarget.style.boxShadow = ''; e.currentTarget.style.borderColor = `${color}33`; }}
    >
      <div style={{
      ...
      <div>
        ...
        {onClick && (
          <div style={{ fontSize: 10, color: `${color}99`, marginTop: 4, fontWeight: 600, letterSpacing: '0.05em', textTransform: 'uppercase' }}>
            View in AI →
          </div>
        )}
      </div>
...
export default function AdminSupervisionTab({ onNavigateToClinicalAI }) {
  ...
  // ── Actions ─────────────────────────────────────────────────────────────────
  const handleAction = async (relId, newStatus) => {
  ...
  const filtered = statusFilter === 'all'
    ? relationships
    : relationships.filter(r => r.status === statusFilter);
  ...
  const s = {
    ...
    alertBox: {
      background: '#1c1208', border: '1px solid #f59e0b44',
      borderRadius: 12, padding: '14px 20px', marginBottom: 24,
    },
    alertTitle: { color: '#fbbf24', fontWeight: 600, fontSize: 14, marginBottom: 10 },
    alertItem: {
      fontSize: 13, color: '#d4a44c', padding: '4px 0',
    ...
    th: {
      textAlign: 'left', padding: '10px 14px', color: 'var(--color-text-secondary)',
    ...
    emptyState: {
      textAlign: 'center', padding: '48px 0', color: 'var(--color-text-secondary)', fontSize: 14,
    },
  };
  ...
  return (
    <div style={s.root}>
      ...
      {/* KPI Cards */}
      <div style={s.kpiRow}>
        <KpiCard label="Active"  value={counts.active  || 0} color="var(--color-success)" icon="✅" onClick={() => onNavigateToClinicalAI?.({ filter: 'active',  label: 'Active Relationships',  icon: '✅', count: counts.active  || 0, note: 'Patients with active medical supervision' })} />
        <KpiCard label="Pending" value={counts.pending || 0} color="#f59e0b" icon="⏳" onClick={() => onNavigateToClinicalAI?.({ filter: 'pending', label: 'Pending Relationships', icon: '⏳', count: counts.pending || 0, note: 'Awaiting confirmation from both parties' })} />
        <KpiCard label="Paused"  value={counts.paused  || 0} color="#6366f1" icon="⏸" onClick={() => onNavigateToClinicalAI?.({ filter: 'paused',  label: 'Paused Relationships',  icon: '⏸', count: counts.paused  || 0, note: 'Supervision temporarily suspended' })} />
        <KpiCard label="Revoked" value={counts.revoked || 0} color="var(--color-danger)" icon="🚫" onClick={() => onNavigateToClinicalAI?.({ filter: 'revoked', label: 'Revoked Relationships', icon: '🚫', count: counts.revoked || 0, note: 'Relationships revoked by admin or physician' })} />
        <KpiCard label="Total"   value={relationships.length} color="var(--color-text-tertiary)" icon="🔗" onClick={() => onNavigateToClinicalAI?.({ filter: 'all',     label: 'All Relationships',     icon: '🔗', count: relationships.length,  note: 'Global overview of the supervision system' })} />
      </div>

      {/* Alerts */}
      {alerts.length > 0 && (
        <div style={s.alertBox}>
          <div style={s.alertTitle}>⚠️ Bottleneck Alerts — {alerts.length} pending relationship{alerts.length > 1 ? 's' : ''} older than 7 days</div>
          {alerts.map(r => (
                ...
                return (
                  <tr key={r.id}>
                    <td style={s.td(i)}>
                      ...
                      <div style={{ color: 'var(--color-text-secondary)', fontSize: 11 }}>{userMap[r.patientId]?.email}</div>
                    </td>
                    <td style={s.td(i)}>
                      <div style={{ fontWeight: 600 }}>{userName(r.doctorId)}</div>
                      <div style={{ color: 'var(--color-text-secondary)', fontSize: 11 }}>{userMap[r.doctorId]?.email}</div>
                    </td>
                    <td style={s.td(i)}><StatusBadge status={r.status} /></td>
                    <td style={s.td(i)}>
                      <span style={{ textTransform: 'capitalize', color: 'var(--color-text-tertiary)' }}>
                        {r.initiatedByRole}
                      </span>
                    </td>
                    <td style={s.td(i)}>
                      <span style={{ color: age >= 7 && r.status === 'pending' ? '#f59e0b' : 'var(--color-text-secondary)' }}>
                        {age !== null ? `${age}d` : '—'}
                      </span>
                    </td>
                    <td style={s.td(i)}>
