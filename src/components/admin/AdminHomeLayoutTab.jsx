          ...
          {loading && versions.length === 0 ? (
            ...
            >
              ...
              <span style={{ fontSize: '0.78rem' }}>
                Save a layout to create the first snapshot.
              </span>
            </div>
          ) : (
              ...
              {versions.map((v, idx) => (
                ...
                >
                  ...
                  <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>
                    Admin: {v.admin?.filter((s) => s.enabled).length ?? '?'} · Clinic:{' '}
                    {v.clinic?.filter((s) => s.enabled).length ?? '?'} · Patient:{' '}
                    {v.patient?.filter((s) => s.enabled).length ?? '?'} · Guest:{' '}
                    {v.guest?.filter((s) => s.enabled).length ?? '?'} visible
                  </div>
                </div>
              ))}
            </div>
    ...
    >
      ...
      >
        ...
        <div style={{ overflowY: 'auto', padding: '1rem 1.5rem', flex: 1 }}>
          ...
          >
            {visibleSections.length} VISIBLE SECTION{visibleSections.length !== 1 ? 'S' : ''} ·{' '}
...
function AudiencePanel({ title, icon: Icon, accent, sections, onChange }) {
  ...
  const handleVisibility = useCallback(
    (idx, value) => {
      if (sections[idx].locked) return;
      const next = sections.map((s, i) => (i === idx ? { ...s, visibility: value } : s));
      onChange(next);
    },
    [sections, onChange]
  );
...
export default function AdminHomeLayoutTab() {
  ...
  return (
    <div>
      ...
      {/* ── Preview Modal ── */}
      {previewing && <LayoutPreviewModal layout={working} onClose={() => setPreviewing(false)} />}
      ...
      >
        ...
        <ul style={{ margin: '0.5rem 0 0', paddingLeft: '1.2rem' }}>
          <li>
            Changes to <code>config/homeLayout</code> take effect immediately across the site.
          </li>
          <li>
            Sections marked <strong>LOCKED</strong> are mandatory and cannot be disabled.
          </li>
          <li>Reordering is disabled while searching to prevent accidental placement errors.</li>
          <li>
            Use <strong>Sync</strong> to quickly duplicate a layout between audiences.
          </li>
        </ul>
      </div>
    </div>
