...
import {
  TrendingUp, DollarSign, PackageSearch, RefreshCw,
  AlertCircle, ChevronDown, ChevronUp, BarChart2,
  ArrowUpRight, ArrowDownRight, Sparkles
} from 'lucide-react';

// ── CF endpoint (same region as other agents) ─────────────────────────────────
const FINANCE_CF_URL = 'https://europe-west1-med-peptides-app.cloudfunctions.net/financeAiAssistant';

// Preset query that triggers the structured snapshot response
const SNAPSHOT_QUERY = 'Give me a complete portfolio financial snapshot: avg retail margin, top 5 products by margin, total gross profit, and any products with margin below 40%.';
...
function KpiPill({ icon, label, value, color }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: '0.68rem', fontWeight: 700, color: 'var(--color-text-tertiary)', letterSpacing: '0.03em' }}>
        {label}
      </div>
    </div>
  );
...
function ProductMarginRow({ p, rank }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ width: 22, height: 22, borderRadius: 'var(--radius-sm)', flexShrink: 0,
        background: `${color}15`, display: 'flex', alignItems: 'center',
        justifyContent: 'center', fontSize: '0.65rem', fontWeight: 900, color }}>
        {rank}
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', flexShrink: 0 }}>
        <div style={{ width: 60, height: 5, borderRadius: '3px', background: 'var(--color-border)', overflow: 'hidden' }}>
          <div style={{ height: '100%', width: `${Math.min(mg ?? 0, 100)}%`,
            background: color, borderRadius: '3px', transition: 'width 0.5s ease' }} />
        </div>
        <span style={{ fontSize: '0.78rem', fontWeight: 800, color, minWidth: 38, textAlign: 'right' }}>
...
export default function AdminFinanceWidget() {
  ...
  // Auto-fetch on mount
  useEffect(() => { fetchSnapshot(); }, []); // eslint-disable-line

  // Extract structured sections from formattedData
  const kpis        = data?.sections?.find(s => s.type === 'finance_kpi_row')?.kpis || [];
  const topProducts = data?.sections?.find(s => s.type === 'finance_top_products')?.products || [];
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid #f1f5f9',
        ...
        <div>
          <h3 style={{ margin: 0, fontSize: '1.05rem', fontWeight: 800, color: '#0f172a',
            ...
            <span style={{ fontSize: '0.58rem', fontWeight: 900, padding: '0.15rem 0.5rem',
              borderRadius: 'var(--radius-sm)', background: 'rgba(139,92,246,0.1)', color: '#8b5cf6',
              border: '1px solid rgba(139,92,246,0.2)', letterSpacing: '0.04em' }}>
              <Sparkles size={8} style={{ display: 'inline', marginRight: 2 }} />AI
            </span>
          </h3>
        ...
        <button onClick={fetchSnapshot} disabled={loading} style={{
          ...
          border: '1px solid #e2e8f0', background: 'var(--color-bg-surface)',
        ...
        >
          <RefreshCw size={13} style={{ animation: loading ? 'finSpin 1s linear infinite' : 'none' }} />
      ...
      <div style={{ padding: '1.25rem 1.5rem' }}>
        ...
        {loading && !data && (
          <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1rem' }}>
            {[1,2,3].map(i => (
              <div key={i} style={{ flex: 1, height: 80, borderRadius: 'var(--radius-md)',
                background: 'var(--surface)',
                backgroundSize: '200% 100%', animation: 'finShimmer 1.5s infinite' }} />
            ))}
          </div>
        )}
        ...
        {kpis.length > 0 && (
          <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
            {kpis.map(k => (
              <KpiPill key={k.label} icon={k.icon} label={k.label}
                value={k.value} color={colorForLabel(k.color)} />
            ))}
          </div>
        )}
        ...
        {topProducts.length > 0 && (
          <div style={{ marginBottom: '1rem' }}>
            <div style={{ fontSize: '0.7rem', fontWeight: 800, color: 'var(--color-text-tertiary)',
        ...
        {weakItems.length > 0 && (
          <div style={{ padding: '0.75rem 1rem', borderRadius: 'var(--radius-md)',
            ...
            marginBottom: '1rem' }}>
            <div style={{ fontSize: '0.72rem', fontWeight: 800, color: 'var(--color-danger)', marginBottom: '0.4rem',
              display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
              <AlertCircle size={12} /> {weakItems.length} producto{weakItems.length > 1 ? 's' : ''} con margen bajo (&lt;40%)
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.3rem' }}>
              {weakItems.map((p, i) => (
                <span key={i} style={{ fontSize: '0.68rem', fontWeight: 700,
        ...
        {rawText && (
          <div>
            <button onClick={() => setExpanded(v => !v)} style={{
              ...
              fontSize: '0.75rem', fontWeight: 700, color: 'var(--color-text-secondary)', marginBottom: '0.5rem',
            }}>
            ...
            {expanded && (
              <div style={{
                ...
                fontSize: '0.78rem', color: 'var(--color-text-secondary)', lineHeight: 1.65,
        ...
        {/* No data yet */}
        {!loading && !data && !error && (
          <div style={{ textAlign: 'center', color: 'var(--color-text-tertiary)', fontSize: '0.85rem', padding: '1.5rem 0' }}>
      ...
      {lastFetched && (
        <div style={{ padding: '0.5rem 1.5rem', borderTop: '1px solid #f1f5f9',
          fontSize: '0.65rem', color: 'var(--color-border)', fontWeight: 600 }}>
