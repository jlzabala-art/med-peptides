...
export default function CuriosityMap({ logs, products, supplements, protocols }) {
  ...
  const freqList = computeItemFrequencies(logs, items, type).filter(x => x.count > 0).slice(0, 10);
  const max = freqList[0]?.count || 1;
  const COLORS = [
    '#8b5cf6', '#6d28d9', '#7c3aed', '#a78bfa', '#5b21b6',
    'var(--color-primary)', 'var(--color-primary)', 'var(--color-primary-hover)', '#60a5fa', '#1e3a8a'
  ];
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
          {freqList.map(({ name, count }, i) => (
            <div key={name} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              ...
              <div style={{ flex: 1, height: '10px', borderRadius: 'var(--radius-sm)', background: 'var(--border)', overflow: 'hidden' }}>
                <div style={{
                  height: '100%',
                  width: `${(count / max) * 100}%`,
                  borderRadius: 'var(--radius-sm)',
              ...
              <div style={{ width: '28px', fontSize: '0.72rem', fontWeight: 800, color: 'var(--text-muted)', flexShrink: 0 }}>
                {count}
              </div>
            </div>
          ))}
