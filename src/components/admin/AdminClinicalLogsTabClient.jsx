...
export default function AdminClinicalLogsTabClient({ initialLogs = [], isSubTab = false }) {
  ...
  const sessions = Array.from(new Set(initialLogs.map((l) => l.sessionId))).map((sid) => {
    const sessionLogs = initialLogs.filter((l) => l.sessionId === sid);
  ...
  const columns = [
      ...
      render: (s) => (
        <div style={{ display: 'inline-flex', flexDirection: 'column', gap: '4px' }}>
          ...
          <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
            <span style={{ opacity: 0.5 }}>↳</span> ID: <CopyableId value={s.id} />
          </div>
        </div>
      ),
    },
    ...
    },
    {
      ...
      header: 'Date',
      width: '140px',
      sortValue: (s) => s.timestamp?.seconds ? s.timestamp.seconds : new Date(s.timestamp).getTime() / 1000,
      render: (s) => (
    ...
    <div style={{ padding: '0.5rem' }}>
      ...
      <div className="kpi-scroll-row" style={{ marginBottom: '1.5rem' }}>
        <MetricCard
          ...
          subtitle="All recorded AI interactions"
          icon={Activity}
          color="#2563eb"
        />
        <MetricCard
          ...
          subtitle="Individual messages processed"
          icon={MessageSquare}
          color="#059669"
        />
        <MetricCard
          title="Sessions with Errors"
          value={sessions.filter(s => s.hasError).length}
          subtitle="Failed or timed out"
          icon={ShieldCheck}
          color="#dc2626"
          alert={sessions.filter(s => s.hasError).length > 0}
        />
        <MetricCard
          title="Unique Users"
      ...
      <div style={{ position: 'fixed', bottom: '1rem', right: '1rem', fontSize: '0.7rem', color: 'var(--text-muted)', opacity: 0.8, background: 'var(--surface)', padding: '4px 8px', borderRadius: '4px', border: '1px solid var(--border)', pointerEvents: 'none', zIndex: 1000, boxShadow: 'var(--shadow-sm)' }}>
        Widget: AdminClinicalLogsTabClient | Props: initialLogs
      </div>
    </div>
  );
}
