...
export default function AdminVariantsTab() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      groups.sort((a, b) =>
        (a.product.displayName ?? a.product.name ?? '').localeCompare(
  ...
  async function handleSave(productId, variantId) {
    const k = editKey(productId, variantId);
    const patch = edits[k];
    if (!patch) return;
  ...
  const filteredGroups = productGroups.filter((g) => {
    ...
    const matchesProduct =
      ...
      (g.product.displayName || '').toLowerCase().includes(term);
    const matchesVariant = g.variants.some(
      (v) =>
        (v.sku || '').toLowerCase().includes(term) || (v.label || '').toLowerCase().includes(term)
    );
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (g) => (
        <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{g.product.category}</span>
      ),
    },
    {
      key: 'variants',
    ...
    <div>
      ...
      >
        <span style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
          <DollarSign size={13} /> 4 price tiers: Guest · Clinic · Wholesale · Pro (USD)
        </span>
        <span style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
          ...
          return (
            ...
            >
              <table className="gcp-table">
                <thead>
                  <tr
                    style={{ backgroundColor: '#f1f5f9', borderBottom: '1px solid var(--border)' }}
                  >
                    {[
                      'SKU',
                      'Label / Strength',
                      ...
                      >
                        <td style={{ padding: '0.5rem 0.6rem', minWidth: '100px' }}>
                          <input
                            ...
                            placeholder={v.id}
                            onChange={(ev) =>
                              setEditField(product.id, v.id, 'sku', ev.target.value)
                            }
                            style={{
                              ...inputStyle,
                        ...
                        <td style={{ padding: '0.5rem 0.6rem', minWidth: '90px' }}>
                          <input
                            ...
                            value={e.guest_usd}
                            onChange={(ev) =>
                              setEditField(product.id, v.id, 'guest_usd', ev.target.value)
                            }
                            style={{ ...inputStyle, borderTop: '2px solid #6366f1' }}
                            title="Retail / Guest price (USD)"
                          />
                        </td>
                        <td style={{ padding: '0.5rem 0.6rem', minWidth: '90px' }}>
                          <input
                            ...
                            value={e.clinic_usd}
                            onChange={(ev) =>
                              setEditField(product.id, v.id, 'clinic_usd', ev.target.value)
                            }
                            style={{ ...inputStyle, borderTop: '2px solid #0ea5e9' }}
                            title="Clinic / Doctor price (USD)"
                          />
                        </td>
                        <td style={{ padding: '0.5rem 0.6rem', minWidth: '90px' }}>
                          <input
                            type="number"
                            step="0.01"
                            value={e.wholesale_usd}
                            onChange={(ev) =>
                              setEditField(product.id, v.id, 'wholesale_usd', ev.target.value)
                            }
                            style={{ ...inputStyle, borderTop: '2px solid #10b981' }}
                            title="Wholesale / Pro price (USD)"
                          />
                        </td>
                        <td style={{ padding: '0.5rem 0.6rem', minWidth: '90px' }}>
                          <input
                            ...
                            value={e.pro_usd}
                            onChange={(ev) =>
                              setEditField(product.id, v.id, 'pro_usd', ev.target.value)
                            }
                            style={{ ...inputStyle, borderTop: '2px solid #f59e0b' }}
                            title="Pro / Internal price (USD)"
                          />
                        </td>
                        <td style={{ padding: '0.5rem 0.6rem', minWidth: '80px' }}>
                          <input
                            ...
                            value={e.stock_qty}
                            onChange={(ev) =>
                              setEditField(product.id, v.id, 'stock_qty', ev.target.value)
                            }
                            style={inputStyle}
                          />
                        </td>
                        <td style={{ padding: '0.5rem 1rem' }}>
                          <button
                            ...
                            onClick={() =>
                              setEditField(product.id, v.id, 'available', !e.available)
                            }
                          >
                          ...
                          {isSaved ? (
                            ...
                            <button
                              ...
                              style={{
                                ...
                                borderRadius: 'var(--radius-sm)',
                                border: 'none',
                                backgroundColor: isDirty ? 'var(--primary)' : 'var(--color-border)',
