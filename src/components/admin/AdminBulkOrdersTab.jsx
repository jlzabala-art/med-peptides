...
import {
  Layers, ChevronDown, ChevronUp, CheckCircle2, Truck, Package,
  Clock, Eye, AlertCircle, Building, ClipboardList, ShoppingBag,
  PackageSearch, Filter, RefreshCw, ArrowRight, Plus
} from 'lucide-react';
...
function BulkBadge({ status }) {
  ...
  return (
    <span style={{
      ...
      background: m.bg, color: m.color, fontSize: '0.67rem', fontWeight: 800,
        ...
        return (
          <React.Fragment key={s}>
            ...
            {i < STATUS_STEPS.length - 1 && (
              <ArrowRight size={10} color={i < idx ? 'var(--color-border)' : 'var(--color-border)'} />
            )}
          </React.Fragment>
        );
      })}
    ...
    <div style={{ marginTop: '0.35rem', display: 'flex', flexDirection: 'column', gap: '0.2rem' }}>
      {sources.map((s, i) => (
        <div key={i} style={{ fontSize: '0.65rem', color: 'var(--color-text-tertiary)', fontWeight: 600,
          paddingLeft: '1.25rem' }}>
          {s.type === 'own'          ? `📦 Stock propio`
           : s.type === 'prescription' ? `💊 Rx: ${s.patientName || '—'} (Dr. ${s.doctorName || '—'})`
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
      {(items || []).map((item, i) => {
        ...
        return (
          <div key={key} style={{ padding: '0.6rem 0.85rem', borderRadius: 'var(--radius-sm)',
            background: 'var(--color-bg-app)', border: '1px solid #f1f5f9' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <span>{item.type === 'protocol' ? '🧬' : '💊'}</span>
              <span style={{ fontWeight: 800, fontSize: '0.82rem', color: 'var(--color-text-primary)', flex: 1 }}>
                {item.name || item.id}
              </span>
              <span style={{ fontWeight: 900, fontSize: '0.95rem', color: 'var(--color-primary)', minWidth: 28, textAlign: 'right' }}>
                {item.quantity}
              </span>
              ...
              {item.sources?.length > 0 && (
                <button onClick={() => toggle(key)}
                  style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-tertiary)', padding: '0.15rem' }}>
      ...
      <div style={{ textAlign: 'right', fontSize: '0.7rem', fontWeight: 700, color: 'var(--color-text-secondary)', marginTop: '0.25rem' }}>
        {(items || []).length} líneas ·{' '}
        {(items || []).reduce((s, i) => s + (i.quantity || 0), 0)} unidades totales
      </div>
    </div>
  );
}
...
function BulkActions({ order, onUpdate }) {
  ...
  const next = {
    submitted: { label: 'Confirmar pedido',   nextStatus: 'confirmed', icon: CheckCircle2, color: 'var(--color-success)' },
    confirmed: { label: 'Marcar enviado',      nextStatus: 'shipped',   icon: Truck,         color: '#f59e0b' },
    shipped:   { label: 'Marcar entregado',    nextStatus: 'delivered', icon: Package,        color: '#6366f1' },
  }[order.status];
  ...
  return (
    <button onClick={handle} disabled={acting} style={{
      display: 'flex', alignItems: 'center', gap: '0.5rem',
      padding: '0.6rem 1.1rem', borderRadius: '9px',
      border: 'none', background: next.color, color: 'var(--color-bg-surface)',
      cursor: acting ? 'not-allowed' : 'pointer', fontFamily: 'inherit',
      fontWeight: 800, fontSize: '0.78rem', opacity: acting ? 0.7 : 1,
      boxShadow: `0 4px 12px ${next.color}40`, transition: 'all 0.15s',
    }}>
      <next.icon size={13} />
...
function BulkOrderCard({ order, onUpdate }) {
  ...
  const createdDate = order.createdAt?.toDate
    ? order.createdAt.toDate().toLocaleDateString('es-ES', { day: '2-digit', month: 'short', year: 'numeric' })
    : '—';
  const submittedDate = order.submittedAt?.toDate
    ? order.submittedAt.toDate().toLocaleString('es-ES', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })
    : '—';
    ...
    <div style={{
      ...
      {isNew && (
        <div style={{ background: 'var(--surface)',
          padding: '0.3rem 1.25rem', fontSize: '0.65rem', fontWeight: 800,
          color: 'var(--color-bg-surface)', letterSpacing: '0.07em' }}>
      ...
      <div style={{ padding: '1rem 1.25rem', display: 'flex', alignItems: 'center', gap: '0.75rem',
        cursor: 'pointer', background: isNew ? 'rgba(99,102,241,0.02)' : 'var(--color-bg-surface)' }}
        onClick={() => setOpen(v => !v)}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: '0.67rem', color: 'var(--color-text-tertiary)', fontWeight: 600, marginTop: '0.1rem',
      ...
      {open && (
        <div style={{ padding: '0 1.25rem 1.25rem', borderTop: '1px solid #f8fafc' }}>
          ...
          {/* Notes */}
          {order.notes && (
            <div style={{ padding: '0.75rem 1rem', borderRadius: 'var(--radius-sm)',
          ...
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
            {order.source_prescription_ids?.map(id => (
              <span key={id} style={{ fontSize: '0.65rem', fontWeight: 700, padding: '0.2rem 0.55rem',
                borderRadius: 'var(--radius-sm)', background: 'rgba(0,54,102,0.06)', color: 'var(--color-primary)' }}>
                💊 Rx/{id.slice(0, 8)}…
              </span>
            ))}
            {order.source_order_ids?.map(id => (
              <span key={id} style={{ fontSize: '0.65rem', fontWeight: 700, padding: '0.2rem 0.55rem',
                borderRadius: 'var(--radius-sm)', background: 'rgba(16,185,129,0.06)', color: 'var(--color-success)' }}>
                🛒 Order/{id.slice(0, 8)}…
              </span>
            ))}
          </div>
...
const sLabel = {
  fontSize: '0.63rem', fontWeight: 800, color: 'var(--color-text-tertiary)',
...
export default function AdminBulkOrdersTab() {
  ...
  const handleUpdate = async (orderId, newStatus) => {
    const event = {
      event: newStatus === 'confirmed' ? 'confirmed' : newStatus === 'shipped' ? 'shipped' : 'delivered',
      actorId: 'admin', actorRole: 'admin', note: '', timestamp: new Date().toISOString(),
    };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.75rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <div style={{ width: 42, height: 42, borderRadius: 'var(--radius-md)', background: 'rgba(99,102,241,0.08)',
            display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
            <Layers size={20} color="#6366f1" />
            {unreadCount > 0 && (
              <span style={{ position: 'absolute', top: -4, right: -4, width: 16, height: 16,
                borderRadius: '50%', background: 'var(--color-danger)', color: 'var(--color-bg-surface)',
                fontSize: '0.55rem', fontWeight: 900, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {unreadCount}
              </span>
            )}
          </div>
          <div>
            ...
            <p style={{ margin: 0, fontSize: '0.72rem', color: 'var(--color-text-tertiary)' }}>
              {stats.total} total · {stats.submitted} pendientes · {stats.confirmed} confirmados · {stats.shipped} en tránsito · {stats.delivered} entregados
            </p>
          </div>
        </div>
        <button onClick={() => setShowBuilder(true)} className="btn btn-primary" style={{
          display: 'flex', alignItems: 'center', gap: '0.4rem',
          fontSize: '0.78rem', fontWeight: 800, padding: '0.55rem 1rem',
      ...
      <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
        {[
          ...
          { key: 'confirmed', label: `Confirmado (${stats.confirmed})`,        color: 'var(--color-success)' },
          { key: 'shipped',   label: `En tránsito (${stats.shipped})`,         color: '#f59e0b' },
          { key: 'delivered', label: `Entregado (${stats.delivered})`,         color: 'var(--color-success)' },
        ].map(f => (
          <button key={f.key} onClick={() => setFilter(f.key)} style={{
            padding: '0.35rem 0.85rem', borderRadius: 'var(--radius-sm)', cursor: 'pointer',
            ...
            fontSize: '0.72rem', fontWeight: 700, transition: 'all 0.12s', fontFamily: 'inherit',
          }}>
            {f.label}
          </button>
        ))}
      ...
      {loading ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
          {[1,2].map(i => (
            <div key={i} style={{ height: 80, borderRadius: '18px',
              background: 'var(--surface)',
              backgroundSize: '200% 100%', animation: 'bkAdShimmer 1.5s infinite' }} />
          ))}
        </div>
      ) : filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '4rem', color: 'var(--color-text-tertiary)' }}>
          ...
          <div style={{ fontWeight: 700, fontSize: '0.9rem' }}>
            {filterStatus !== 'all' ? 'No hay bulk orders con ese estado.' : 'Aún no hay bulk orders de wholesalers.'}
          </div>
