...
import { createRelationship, getAllRelationships, updateRelationshipStatus } from '../../services/assignmentService';
import { Stethoscope, User, Link2, CheckCircle2, XCircle, Clock, RefreshCw, AlertTriangle } from 'lucide-react';
...
export default function AdminAssignPhysicianTab({ adminUid }) {
  ...
  const handleAssign = async (e) => {
    ...
    try {
      ...
      setPatientId(''); setPhysicianId(''); setNotes('');
      await fetchRels();
    } catch (e) {
  ...
  // ── revoke ────────────────────────────────────────────────────────────────
  const handleRevoke = async (relId) => {
  ...
  return (
    <div className="anim-slide-up" style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      <div className="card" style={{ padding: '1.75rem' }}>
        <h2 style={{ fontSize: '1.1rem', fontWeight: 900, marginBottom: '0.25rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
        ...
        {error   && <div style={{ background: '#fee2e2', color: '#b91c1c', padding: '0.75rem 1rem', borderRadius: 'var(--radius-sm)', fontSize: '0.85rem', marginBottom: '1rem' }}><AlertTriangle size={14} style={{ marginRight: 6 }} />{error}</div>}
        {success && <div style={{ background: '#dcfce7', color: 'var(--color-success)', padding: '0.75rem 1rem', borderRadius: 'var(--radius-sm)', fontSize: '0.85rem', marginBottom: '1rem' }}>{success}</div>}
        ...
        <form onSubmit={handleAssign} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            ...
            <div>
              <label style={{ fontSize: '0.78rem', fontWeight: 700, color: 'var(--text-muted)', display: 'block', marginBottom: '0.4rem' }}>
                <User size={12} style={{ marginRight: 4 }} />PATIENT
              </label>
              <select
                value={patientId}
                onChange={e => setPatientId(e.target.value)}
                disabled={loadingUsers}
                style={{ width: '100%', padding: '0.6rem 0.9rem', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)', fontSize: '0.88rem', backgroundColor: 'white' }}
              >
            ...
            <div>
              <label style={{ fontSize: '0.78rem', fontWeight: 700, color: 'var(--text-muted)', display: 'block', marginBottom: '0.4rem' }}>
                <Stethoscope size={12} style={{ marginRight: 4 }} />DOCTOR
              </label>
              <select
                value={doctorId}
          ...
          {/* Notes */}
          <div>
            <label style={{ fontSize: '0.78rem', fontWeight: 700, color: 'var(--text-muted)', display: 'block', marginBottom: '0.4rem' }}>NOTES (optional)</label>
            <input
              type="text"
              value={notes}
              onChange={e => setNotes(e.target.value)}
              placeholder="e.g. referred by clinic, protocol: BPC-157"
              style={{ width: '100%', padding: '0.6rem 0.9rem', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)', fontSize: '0.88rem' }}
            />
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
            <button
              ...
              style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', opacity: (!patientId || !doctorId) ? 0.5 : 1 }}
            >
      ...
      <div className="card" style={{ padding: '1.75rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
          ...
          <button onClick={fetchRels} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '0.35rem', fontSize: '0.8rem' }}>
            ...
            <table className="gcp-table">
              <thead>
                <tr style={{ borderBottom: '2px solid var(--border)' }}>
                  {['Patient', 'Physician', 'Status', 'Initiated By', 'Date', 'Notes', ''].map(h => (
                    <th key={h} style={{ textAlign: 'left', padding: '0.5rem 0.75rem', fontSize: '0.7rem', fontWeight: 800, color: 'var(--text-muted)', textTransform: 'uppercase' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {relationships.map(rel => (
                  <tr key={rel.id} style={{ borderBottom: '1px solid var(--border-light)' }}>
                    <td style={{ padding: '0.65rem 0.75rem', fontWeight: 700 }}>{userName(rel.patientId)}</td>
                    <td style={{ padding: '0.65rem 0.75rem' }}>{userName(rel.doctorId)}</td>
                    <td style={{ padding: '0.65rem 0.75rem' }}>{statusBadge(rel.status)}</td>
                    <td style={{ padding: '0.65rem 0.75rem', color: 'var(--text-muted)' }}>{rel.initiatedByRole}</td>
                    ...
                    <td style={{ padding: '0.65rem 0.75rem', color: 'var(--text-muted)', maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {rel.notes || '—'}
                    </td>
                    <td style={{ padding: '0.65rem 0.75rem' }}>
                      {rel.status !== 'revoked' && (
                        <button
                          ...
                          style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-danger)', display: 'flex', alignItems: 'center', gap: '0.3rem', fontSize: '0.75rem', fontWeight: 700 }}
                        >
