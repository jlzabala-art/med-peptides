...
import { useRouter as useNav, useSearchParams as useSP } from 'next/navigation';
...
const ProductMicrosite = dynamic(() => import('./products/ProductMicrosite'));
const CreateProductModal = dynamic(() => import('./CreateProductModal'));
const BulkOrderSelectionModal = dynamic(() => import('./BulkOrders/BulkOrderSelectionModal'));
...
export default function AdminProductsTableClient({
  ...
  const { products, loading, loadingMore, fetchProducts, loadMore, hasMore, totalCount } =
    useProducts(allowedCategories, { initialData: initialProducts });
  ...
  const handleCreatePrescription = (selectedIds) => {
    ...
    router.push(`/prescriptions/new?source=Selected Items&items=${selectedIds.join(',')}`);
  };
    ...
    <div style={{ marginBottom: '2rem' }}>
      ...
      <div style={{ marginBottom: '1rem' }}>
        <GlobalSearchBar
          ...
          placeholder="Search catalog by name, sku, category (Algolia)..."
          resultCount={loading && !algoliaLoading ? undefined : Object.keys(filteredGroups).length}
          ...
          <>
            {viewMode === 'grid' ? (
              ...
              <DataTable
                ...
                emptyDescription="No products match the selected filters or search criteria."
              />
            )}
            {hasMore && (
      ...
      <BulkActionsBar
        ...
        actions={[
          { label: 'Add to Bulk Order', icon: <ShoppingCart size={14} />, onClick: () => { setProductsToBulkOrder(selectedProductIds); setIsBulkOrderModalOpen(true); } },
          { label: 'Create Prescription', icon: <ClipboardList size={14} />, onClick: () => router.push(`/admin/prescriptions/new?items=${selectedProductIds.join(',')}`) },
          { label: 'Deactivate', icon: <XCircle size={14} />, onClick: () => handleDeactivateSelected(selectedProductIds) },
          { label: 'Export Selected', icon: <Download size={14} />, onClick: handleExportCSV },
          ...(!readOnly ? [
            { label: 'Bulk Price Update', icon: <Percent size={14} />, onClick: () => setBulkMode(!bulkMode) },
            { label: 'Include in Catalog', icon: <BookOpen size={14} />, onClick: () => setCatalogSelectMode(true) },
            { label: 'Manage Visibility', icon: <Eye size={14} />, onClick: () => handleManageVisibility(selectedProductIds) }
          ] : [])
        ]}
      />
