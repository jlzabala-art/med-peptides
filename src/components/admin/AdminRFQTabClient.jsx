...
import { Card, MetricCard, StatusChip } from '../ui';
...
import { FileText, Loader2, Plus, Sparkles, CheckCircle, AlertTriangle, Send, Receipt, Download, Activity, Eye, Link, FileCheck, ShoppingCart } from '@/lib/icons';
import AdminTabErrorBoundary from './AdminTabErrorBoundary';
import AppActionGroup from '../ui/AppActionGroup';
export default function AdminRFQTabClient({ initialRfqs = [], isSubTab = false }) {
  const { user } = useAuth();
  ...
  // Initial fetch omitted since it's passed as prop, but can be triggered manually.
  ...
  const columns = [
    ...
    },
    {
      ...
      sortKey: 'status',
      render: (r) => (
        <StatusChip status={r.status?.toLowerCase() || 'draft'} />
      )
    },
    {
      key: 'poAttached',
      ...
      render: (r) => (
        <label style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', fontSize: '0.8rem' }}>
          <input 
            type="checkbox" 
      ...
      render: (r) => (
        <div style={{ display: 'inline-flex', justifyContent: 'flex-end', width: '100%' }}>
          <AppActionGroup actions={[
            { type: 'custom', icon: Sparkles, label: 'AI Audit', onClick: () => {
                const itemNames = (r.items || []).map(i => i.itemName || i.productName || i.name || i.peptide_name).filter(Boolean).join(', ');
                window.dispatchEvent(new CustomEvent('open-clinical-ai', {
                  detail: {
                    message: `Analyze this RFQ for ${r.clientName || 'Client'} from supplier ${r.supplierName || 'Unknown'}. It includes: ${itemNames}. Verify standard technical specifications, COA requirements, and expected purity.`,
                    autoSend: true
                  }
                }));
            } },
            { type: 'custom', icon: Eye, onClick: () => { setPreviewData(r); setPreviewType('supplier'); }, label: 'Preview Supplier' },
            { type: 'custom', icon: Eye, onClick: () => { setPreviewData(r); setPreviewType('client'); }, label: 'Preview Client' },
            { type: 'custom', icon: Link, onClick: () => generateSupplierMagicLink(r.id), label: 'Supplier Link' },
            { type: 'custom', icon: Link, onClick: () => generateClientMagicLink(r.id), label: 'Client Link' },
            ...(r.poAttached ? [{
              type: 'custom',
              icon: Receipt,
              label: 'Upload Invoice',
              onClick: () => document.getElementById(`invoice-upload-${r.id}`)?.click()
            }] : []),
            ...
            ...(r.status === 'DISCREPANCY_FLAGGED' ? [
              { type: 'custom', icon: FileCheck, label: 'Re-Run Audit', onClick: () => reconcileInvoice(r.id), disabled: isReconciling },
              { type: 'custom', icon: AlertTriangle, label: 'View Discrepancies', onClick: () => { setReconciliationResult(r.invoiceReconciliation); setSelectedRfqId(r.id); } }
            ] : [])
          ]} />
          {r.poAttached && (
            <input 
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        {!isSubTab && (
          <PageHeader
            ...
            subtitle="Manage Request For Quotes, Supplier Negotiations, and Purchasing."
            actions={
              ...
              }} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '13px', padding: '0.4rem 1rem' }}>
        ...
        <div className="tab-container" style={{ padding: '1.5rem', flex: 1, overflowY: 'auto' }}>
          <div className="kpi-scroll-row" style={{ marginBottom: '1.5rem' }}>
            <MetricCard 
              ...
              color="#3b82f6" 
              title="Total RFQs" 
              value={loading ? '-' : rfqs.length} 
            />
            <MetricCard 
              icon={AlertTriangle} 
              color="#d97706" 
              title="Awaiting PO" 
              value={loading ? '-' : rfqs.filter(r => !r.poAttached).length} 
            />
            <MetricCard 
              icon={CheckCircle} 
              color="#16a34a" 
              title="PO Created" 
              value={loading ? '-' : rfqs.filter(r => r.status === 'PO_CREATED').length} 
            />
            <MetricCard 
              icon={Clock} 
              color="#64748b" 
              title="Draft RFQs" 
              value={loading ? '-' : rfqs.filter(r => r.status === 'DRAFT').length} 
            />
          </div>
