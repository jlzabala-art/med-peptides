...
const ROLE_LABELS = {
  ...
  clinic: {
    name: 'Clinic / Group',
    color: '#8b5cf6',
    desc: 'Multi-practitioner Clinic Management',
  },
  doctor: {
    name: 'Practitioner',
    color: 'var(--color-success)',
    desc: 'Prescribing Physician & Protocols',
  },
  wholesaler: { name: 'Wholesaler', color: '#f59e0b', desc: 'Bulk Peptide Sourcing & Logistics' },
  sales_agent: {
    name: 'Sales Agent',
    color: 'var(--color-primary)',
    desc: 'Commission & Client Portfolio Tracking',
  },
  ...
  guest: {
    ...
    desc: 'Anonymous Public Portal Access',
  },
};
...
const ACTION_PERMISSIONS = [
  {
    key: 'canRecommend',
    label: 'Recommend Protocols',
    desc: 'Create tailored recommendations for patients',
  },
  {
    key: 'canBulkOrder',
    label: 'Bulk Orders',
    desc: 'Authorize and execute wholesale bulk transactions',
  },
  {
    key: 'customSynthesis',
    label: 'Custom Peptide Synthesis',
    desc: 'Access proprietary custom synthesis forms',
  },
  {
    key: 'clinicalLogs',
    label: 'Access Clinical Logs',
    desc: 'Review AI system logs and diagnostic audits',
  },
  {
    key: 'manageStaff',
    label: 'Staff Management',
    desc: 'Invite, delegate, and manage supporting roles',
  },
  {
    key: 'trackCommission',
    label: 'Commission Tracking',
    desc: 'Track sales commissions and agent performance',
  },
];

const VIEW_PERMISSIONS = [
  {
    key: 'canAccessAdminDashboard',
    label: 'Access Admin Dashboard',
    desc: 'Full core operations, settings, and logs',
  },
  {
    key: 'canAccessPhysicianDashboard',
    label: 'Access Physician Dashboard',
    desc: 'Patient rosters, prescriptions, and timeline logs',
  },
  {
    key: 'canAccessCalculator',
    label: 'Dose Calculator',
    desc: 'Access reconstituted peptide dose utilities',
  },
  {
    key: 'canAccessAcademy',
    label: 'Knowledge Hub & Academy',
    desc: 'Access scientific blogs and training modules',
  },
  {
    key: 'canAccessClinicalAI',
    label: 'ClinicalAI Assistant',
    desc: 'Access the conversational ClinicalAI engine',
  },
  {
    key: 'canAccessCustomSynthesis',
...
export default function AdminAccessLevelsTab() {
  ...
  async function handleToggle(roleKey, permissionKey) {
  ...
  async function handleResetToDefault() {
    if (
      !window.confirm(
        'Are you sure you want to reset all role permissions to standard default settings?'
      )
    )
      return;
    setLoading(true);
    ...
    const DEFAULT_PERMS = {
      ...
      wholesaler: {
        ...
        trackCommission: false,
        canAccessAdminDashboard: false,
        canAccessPhysicianDashboard: false,
        canAccessCalculator: true,
        canAccessAcademy: true,
  ...
  return (
    <div style={{ paddingBottom: '4rem' }}>
        ...
        columns={[
          {
            ...
            render: (row) => <AppEntityCell title={row.name} />,
          },
          ...
          },
          {
            ...
            render: (row) => (
              <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                <AppActionGroup
                  actions={[{ type: 'edit', onClick: () => setEditingRole(row.roleKey) }]}
                />
              </div>
            ),
          },
              ...
              <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'flex-start' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                  {[...ACTION_PERMISSIONS, ...VIEW_PERMISSIONS]
                    .filter(
                      (p) =>
                        permissions && permissions[row.roleKey] && permissions[row.roleKey][p.key]
                    )
                    .map((p) => (
                      <span
                        key={p.key}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  {ACTION_PERMISSIONS.map((item) => {
                    ...
                    return (
                      ...
                      >
                        <div style={{ flex: 1 }}>
                          <div style={{ fontSize: '0.8rem', fontWeight: 600, color: '#202124' }}>
                            {item.label}
                          </div>
                          <div style={{ fontSize: '0.7rem', color: '#5f6368', marginTop: '2px' }}>
                            {item.desc}
                          </div>
                        </div>
                        <label
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={hasPerm}
                            onChange={() => handleToggle(editingRole, item.key)}
                          ...
                          <span
                            style={{
                              ...
                              right: 0,
                              bottom: 0,
                              backgroundColor: hasPerm ? '#1a73e8' : '#dadce0',
                              transition: '.2s',
                              borderRadius: '8px',
                            }}
                          >
                            <span
                              style={{
                                ...
                                borderRadius: '50%',
                                transition: '.2s',
                                transform: hasPerm ? 'translateX(12px)' : 'translateX(0)',
      ...
      <div style={{ position: 'fixed', bottom: '1rem', right: '1rem', fontSize: '0.7rem', color: 'var(--text-muted)', opacity: 0.8, background: 'var(--surface)', padding: '4px 8px', borderRadius: '4px', border: '1px solid var(--border)', pointerEvents: 'none', zIndex: 1000, boxShadow: 'var(--shadow-sm)' }}>
        Widget: AdminAccessLevelsTab | Props: none
      </div>
