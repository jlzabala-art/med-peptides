...
export default function AdminSemanticTab({ readOnly = false }) {
  ...
  const callRefineAgent = async (body) => {
    const token = await getToken();
    const baseUrl = import.meta.env.VITE_FUNCTIONS_BASE_URL || 'https://europe-west1-med-peptides-app.cloudfunctions.net';
  ...
  const handleSingleRefine = async (product) => {
    setRefiningIds(prev => {
  ...
  const handleBulkRefine = async (idsToRefine) => {
  ...
  const handleSaveManualEdits = async (e) => {
    e.preventDefault();
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', alignItems: 'center' }}>
          {p.goals && p.goals.length > 0 ? (
            p.goals.slice(0, 2).map(goal => (
              <span key={goal} style={{ fontSize: '11px', padding: '2px 6px', background: 'var(--color-bg-hover)', color: 'var(--color-text-primary)', borderRadius: '4px' }}>
                {GOAL_LABELS[goal] || goal}
              </span>
            ))
          ) : (
            <span style={{ fontSize: '12px', color: 'var(--color-text-tertiary)', fontStyle: 'italic' }}>No goals assigned</span>
          )}
          {p.goals && p.goals.length > 2 && (
            <span style={{ fontSize: '11px', color: 'var(--color-primary)', fontWeight: 500 }}>+{p.goals.length - 2} more</span>
          )}
        </div>
      )
    },
      ...
      render: (p) => {
        ...
        return (
          <div style={{ display: 'flex', gap: '6px', justifyContent: 'flex-end' }}>
            <button
              title="Edit manually"
              onClick={(e) => { e.stopPropagation(); setEditingProduct({ ...p, goals: p.goals || [], secondaryFactors: p.secondaryFactors || [], mechanisms: p.mechanisms || [] }); }}
              disabled={isRefining || readOnly}
              style={{ padding: '4px', borderRadius: '4px', border: '1px solid var(--color-border)', background: 'var(--color-bg-surface)', color: 'var(--color-text-secondary)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
            >
              <Edit2 size={13} />
            </button>
            <button
              title="Refine with AI"
              onClick={(e) => { e.stopPropagation(); handleSingleRefine(p); }}
              disabled={isRefining || readOnly}
              style={{ padding: '4px', borderRadius: '4px', border: '1px solid var(--color-border)', background: 'var(--color-bg-surface)', color: 'var(--color-primary)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
            >
              {isRefining ? <Loader2 size={13} className="spinner-small" /> : <Sparkles size={13} />}
            </button>
          </div>
        );
      }
  ...
  const renderExpandedRow = (p) => (
    <div style={{ padding: "16px 24px", background: "var(--color-bg-app)", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "20px" }}>
      <div style={{ background: "var(--color-bg-surface)", border: "1px solid var(--color-border)", borderRadius: "var(--radius-sm)", padding: "14px", display: "flex", flexDirection: "column", boxShadow: "0 1px 2px 0 rgba(60,64,67,0.05)" }}>
        <div style={{ fontSize: "11px", fontWeight: 700, textTransform: "uppercase", color: "var(--color-text-tertiary)", letterSpacing: "0.05em", paddingBottom: "6px", borderBottom: "1px solid var(--color-border)", marginBottom: "10px" }}>Product Description</div>
      ...
      <div style={{ background: "var(--color-bg-surface)", border: "1px solid var(--color-border)", borderRadius: "var(--radius-sm)", padding: "14px", display: "flex", flexDirection: "column", boxShadow: "0 1px 2px 0 rgba(60,64,67,0.05)" }}>
        <div style={{ fontSize: "11px", fontWeight: 700, textTransform: "uppercase", color: "var(--color-text-tertiary)", letterSpacing: "0.05em", paddingBottom: "6px", borderBottom: "1px solid var(--color-border)", marginBottom: "10px" }}>Canonical Optimization Goals</div>
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          {CANONICAL_GOALS.map(goal => {
            const isActive = p.goals?.includes(goal);
            return (
              <div key={goal} style={{ display: "flex", alignItems: "center", fontSize: "13px", color: isActive ? 'var(--color-text-primary)' : 'var(--color-text-tertiary)' }}>
                {isActive ? (
                  <CheckCircle2 size={14} style={{ color: 'var(--color-success)', marginRight: 8 }} />
                ) : (
                  <div style={{ width: "14px", height: "14px", borderRadius: "50%", border: "1px solid var(--color-border)", marginRight: "8px" }} />
                )}
                <span style={{ fontSize: 12, fontWeight: isActive ? 600 : 400 }}>{GOAL_LABELS[goal]}</span>
              </div>
            );
          })}
        </div>
      ...
      <div style={{ background: "var(--color-bg-surface)", border: "1px solid var(--color-border)", borderRadius: "var(--radius-sm)", padding: "14px", display: "flex", flexDirection: "column", boxShadow: "0 1px 2px 0 rgba(60,64,67,0.05)" }}>
        <div style={{ fontSize: "11px", fontWeight: 700, textTransform: "uppercase", color: "var(--color-text-tertiary)", letterSpacing: "0.05em", paddingBottom: "6px", borderBottom: "1px solid var(--color-border)", marginBottom: "10px" }}>Secondary Search Tags (English)</div>
        <div style={{ display: "flex", flexWrap: "wrap", gap: "4px" }}>
          {p.secondaryFactors && p.secondaryFactors.length > 0 ? (
            p.secondaryFactors.map(tag => (
              <span key={tag} style={{ fontSize: "11px", padding: "2px 6px", background: "rgba(26,115,232,0.1)", color: "var(--color-primary)", borderRadius: "4px" }}>
                {tag}
              </span>
            ))
          ) : (
            <span style={{ fontSize: "12px", color: "var(--color-text-tertiary)", fontStyle: "italic" }}>No search tags generated</span>
          )}
        </div>

        <div style={{ fontSize: "11px", fontWeight: 700, textTransform: "uppercase", color: "var(--color-text-tertiary)", letterSpacing: "0.05em", paddingBottom: "6px", borderBottom: "1px solid var(--color-border)", marginBottom: "10px", marginTop: "14px" }}>Biological Mechanisms</div>
        <ul style={{ margin: 0, paddingLeft: "16px", fontSize: "13px", color: "var(--color-text-secondary)", lineHeight: 1.5 }}>
          {p.mechanisms && p.mechanisms.length > 0 ? (
            p.mechanisms.map((mech, index) => (
              <li key={index} style={{ marginBottom: "4px" }}>{mech}</li>
            ))
          ) : (
            <span style={{ fontSize: "12px", color: "var(--color-text-tertiary)", fontStyle: "italic" }}>No biological mechanisms declared</span>
          )}
        </ul>
      </div>
    </div>
  ...
  return (
    <div style={{ padding: "24px", background: "var(--color-bg-surface)" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid var(--color-border)", paddingBottom: "16px", marginBottom: "20px" }}>
        <div>
          <h2 style={{ margin: 0, fontSize: "18px", fontWeight: 600, color: "var(--color-text-primary)", display: "flex", alignItems: "center", gap: "8px" }}>
      ...
      <div style={{ border: '1px solid var(--color-border)', borderRadius: 'var(--table-radius)', backgroundColor: 'var(--color-bg-surface)', overflow: 'hidden' }}>
        <AppDataTable 
          ...
          renderBatchActions={(ids) => (
            <button
              ...
              style={{ padding: '6px 12px', borderRadius: '4px', border: 'none', background: 'var(--color-primary)', color: 'white', cursor: 'pointer', fontSize: '13px', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '6px' }}
            >
              {bulkRefining ? <Loader2 size={14} className="spinner-small" /> : <Sparkles size={14} />}
        ...
        {hasMore && products.length > 0 && !loading && (
          <div style={{ padding: '12px', borderTop: '1px solid var(--color-border)', textAlign: 'center', backgroundColor: 'var(--color-bg-app)' }}>
            <button
      ...
      <div style={{ marginTop: '2rem' }}>
        <div style={{ background: "var(--color-bg-app)", borderRadius: "4px", padding: "14px 18px", border: "1px solid var(--color-border)" }}>
          <div style={{ color: "var(--color-text-secondary)", fontSize: "11px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "8px" }}>
            System Execution Logs
          </div>
          <div style={{ maxHeight: "150px", overflowY: "auto" }}>
            {log.length === 0 ? (
              <div style={{ color: 'var(--color-text-tertiary)', fontSize: 12 }}>Console idle... awaiting requests.</div>
            ) : (
              log.map((entry, idx) => (
                <div key={idx} style={{ fontSize: "12px", fontFamily: "monospace", padding: "2px 0", lineHeight: 1.6, color: entry.type === 'error' ? 'var(--color-danger)' : entry.type === 'success' ? 'var(--color-success)' : 'var(--color-text-primary)' }}>
                  <span style={{ color: "var(--color-text-tertiary)", marginRight: "8px" }}>[{entry.ts}]</span>
                  {entry.msg}
                </div>
              ))
            )}
      ...
      {editingProduct && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(15, 23, 42, 0.4)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "16px" }}>
          <div style={{ background: "var(--color-bg-surface)", borderRadius: "8px", width: "100%", maxWidth: "520px", boxShadow: "0 25px 50px -12px rgba(0,0,0,0.25)", border: "1px solid var(--color-border)", overflow: "hidden" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px", borderBottom: "1px solid var(--color-border)" }}>
              <h3 style={{ margin: 0, fontSize: "15px", fontWeight: 600, color: "var(--color-text-primary)" }}>Edit Semantic Metadata: {editingProduct.name}</h3>
              <button style={{ background: "transparent", border: "none", color: "var(--color-text-tertiary)", cursor: "pointer", padding: "4px" }} onClick={() => setEditingProduct(null)}>
                <X size={18} />
              </button>
            </div>

            <form onSubmit={handleSaveManualEdits} style={{ padding: "16px", display: "flex", flexDirection: "column", gap: "16px" }}>
              <div>
                <label style={{ display: "block", fontSize: "12px", fontWeight: 700, color: "var(--color-text-primary)", marginBottom: "6px" }}>Canonical Optimization Goals</label>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" }}>
                  {CANONICAL_GOALS.map(goal => {
                    const isChecked = editingProduct.goals.includes(goal);
                    return (
                      <label key={goal} style={{ fontSize: "13px", color: "var(--color-text-primary)", display: "flex", alignItems: "center", cursor: "pointer" }}>
                        <input
                          type="checkbox"
                          checked={isChecked}
                          onChange={(e) => {
              ...
              <div>
                <label style={{ display: "block", fontSize: "12px", fontWeight: 700, color: "var(--color-text-primary)", marginBottom: "6px" }}>Secondary Search Tags (English, comma-separated)</label>
                <textarea
                  style={{ width: "100%", fontSize: "13px", padding: "8px", border: "1px solid var(--color-border)", borderRadius: "4px", background: "var(--color-bg-app)", outline: "none", resize: "none", fontFamily: "inherit" }}
                  rows={2}
              ...
              <div>
                <label style={{ display: "block", fontSize: "12px", fontWeight: 700, color: "var(--color-text-primary)", marginBottom: "6px" }}>Biological Mechanisms (comma-separated)</label>
                <textarea
                  style={{ width: "100%", fontSize: "13px", padding: "8px", border: "1px solid var(--color-border)", borderRadius: "4px", background: "var(--color-bg-app)", outline: "none", resize: "none", fontFamily: "inherit" }}
                  rows={2}
