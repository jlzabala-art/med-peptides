  ...
  const [approvals, setApprovals] = useState([
    {
      ...
      detail: 'Regpept Peptides Delivery',
      amount: 18400,
      department: 'R&D',
      status: 'Pending',
    },
    ...
    <div className="anim-fade-up" style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      >
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
          <button
            onClick={() => toast.info('Budget creator workspace under construction')}
          ...
          <button
            onClick={() => toast.info('Import utility open')}
            className="gcp-btn-secondary"
            style={{
              fontSize: '0.75rem',
              padding: '0.4rem 0.8rem',
        ...
        {[
          {
            ...
            value: (
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                <span>
                  {totalSpent.toLocaleString()} / {totalBudget.toLocaleString()} AED
                </span>
                <span style={{ fontSize: '0.65em', color: 'var(--text-muted)' }}>
                  ≈ {Math.round(totalSpent / 3.6725).toLocaleString()} /{' '}
                  {Math.round(totalBudget / 3.6725).toLocaleString()} USD
                </span>
              </div>
            ),
            sub: `${totalPct}% consumed`,
            icon: Landmark,
            color: 'var(--primary)',
            highlight: '#eff6ff',
          },
          ...
          },
          {
            ...
            value: formatAEDtoDual(remainingBudget),
            sub: 'Available Q3 funds',
            icon: Coins,
            color: '#10b981',
            highlight: '#dcfce7',
          },
          {
            key: 'forecast',
            title: 'Forecast End of Q',
            value: formatAEDtoDual(2350, '+'),
            sub: 'Projected overspend',
            icon: ArrowUpRight,
            color: '#ef4444',
            highlight: '#fee2e2',
          },
      ...
      >
        ...
        >
          <div
            className="glass-card-premium"
            style={{ padding: '1.5rem', border: '1px solid #bfdbfe', backgroundColor: '#fafcff' }}
          >
            <div
              style={{
                display: 'flex',
            ...
            >
              {[
                {
                  text: 'Software costs are growing faster than revenue.',
                  severity: 'Red',
                  color: '#ef4444',
                  bg: '#fef2f2',
                },
                {
                  text: 'Current burn rate will exceed budget in 17 days.',
                  severity: 'Red',
                  color: '#ef4444',
                  bg: '#fef2f2',
                },
                {
                  text: 'Marketing spending increased 18% versus last month.',
                  severity: 'Yellow',
                  color: '#d97706',
                  bg: '#fffbeb',
                },
                {
                  text: 'Travel expenses are trending above forecast.',
                  severity: 'Yellow',
                  color: '#d97706',
                  bg: '#fffbeb',
                },
                {
                  text: 'R&D spending remains below planned allocation.',
                  severity: 'Green',
                  color: '#10b981',
                  bg: '#f0fdf4',
                },
                ...
                >
                  ...
                  <span style={{ fontSize: '0.7rem', fontWeight: 700, color: insight.color }}>
                    {insight.severity}
                  </span>
                </div>
              ))}
            ...
            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
              ...
              >
                {generatingReport && aiReportType === 'CFO'
                  ? 'Analyzing...'
              ...
              <button
                ...
                style={{ fontSize: '0.75rem', padding: '0.4rem 0.8rem' }}
              >
                {generatingReport && aiReportType === 'SAVINGS'
                  ? 'Calculating...'
          ...
          >
            ...
            >
              ...
              >
                ...
                <strong style={{ color: zohoStatus.pending > 0 ? '#ea580c' : 'var(--text-main)' }}>
                  {zohoStatus.pending}
                </strong>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ color: 'var(--text-muted)' }}>Failed sync logs:</span>
                <strong style={{ color: zohoStatus.failed > 0 ? '#ef4444' : 'var(--text-main)' }}>
                  {zohoStatus.failed}
                </strong>
              </div>
            </div>
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  <span
                    style={{ fontSize: '1.5rem', fontWeight: '800', color: 'var(--text-main)' }}
                  >
                    {totalPct}%
                  </span>
                  <span
              ...
              <div style={{ flex: 1, width: '100%' }}>
                <DataTable
                  data={Object.entries(budgetMap).map(([name, data]) => {
                    const pct = Math.round((data.spent / data.budget) * 100);
                    const isOver = data.spent > data.budget;
                    const filledBlocks = Math.round(pct / 10);
                    const barString =
                      '█'.repeat(Math.min(filledBlocks, 10)) +
                      '░'.repeat(Math.max(10 - filledBlocks, 0));
                    return { name, ...data, pct, isOver, filledBlocks, barString };
                  })}
                  keyField="name"
                  ...
                  columns={[
                    ...
                    },
                    {
                      ...
                      render: (r) => (
                        <span style={{ fontSize: '0.75rem' }}>{formatCurrency(r.budget)}</span>
                      ),
                    },
                    {
                      key: 'spent',
      ...
      <div style={{ gridColumn: isMobile ? '1' : 'span 4' }}>
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h3
              style={{ margin: 0, fontSize: '0.95rem', fontWeight: 800, color: 'var(--text-main)' }}
            >
            ...
            {[
              ...
              >
                <div>
                  ...
                  >
                    {driver.pct}% of total spent
                  </span>
                </div>
                <span style={{ fontWeight: 700, color: 'var(--text-main)' }}>
                  {driver.cost.toLocaleString()} AED
                </span>
              </div>
            ))}
          </div>
      ...
      >
        ...
        >
          ...
          <div
            style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.8rem' }}
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
