...
function FrictionInterpretation({ friction }) {
  ...
  const ratio =
    totalEmptyEvents > 0 && totalRepeatedEvents > 0
      ? (totalRepeatedEvents / totalEmptyEvents).toFixed(1)
      : null;

  if (ratio && ratio >= 1.5) {
    insights.push(
      `Repeated searches are ${ratio}× more frequent than empty results — users are finding *some* content but not satisfying their intent. Focus on content quality over discoverability.`
    );
  }
  ...
  if (insights.length === 0) {
    ...
    insights.push(
      'Friction signals detected. Accumulating more data to generate specific recommendations.'
    );
  }
