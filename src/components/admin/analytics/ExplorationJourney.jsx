...
import { Search, FlaskConical, BookOpen, ShoppingCart, ArrowRight, Compass, Zap } from 'lucide-react';
...
function StageCard({ stage, items, cartVal, isLast }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'stretch', flex: 1, minWidth: 0 }}>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
          <div style={{
            ...
            padding: '0.45rem', borderRadius: 'var(--radius-sm)', display: 'flex', flexShrink: 0
          }}>
            ...
            {cartFormatted ? (
              <>
                <span style={{
                  ...
                  color: stage.color, letterSpacing: '-0.03em', lineHeight: 1
                }}>
                  {cartFormatted}
                </span>
      ...
      {!isLast && (
        <div style={{
          display: 'flex', alignItems: 'center', padding: '0 0.4rem',
...
function DepthSignal({ tops }) {
  ...
  const depthColor = depth === 3 ? 'var(--color-success)' : depth === 2 ? '#0891b2' : depth === 1 ? '#f59e0b' : 'var(--text-light)';
...
export default function ExplorationJourney({ tops = {}, funnel = [] }) {
  ...
  const cartStep = funnel.find(s =>
    s.label?.toLowerCase().includes('cart') ||
    s.label?.toLowerCase().includes('add to')
  );
  const cartVal = cartStep?.value
    ? (typeof cartStep.value === 'object' ? cartStep.value.current : cartStep.value)
    : null;
