...
import { ref, listAll, uploadBytesResumable, getDownloadURL, deleteObject, uploadString } from 'firebase/storage';
import { Database, Folder, File as FileIcon, UploadCloud, FolderPlus, Trash2, ChevronRight, HardDrive, FileText, Download, AlertTriangle, X } from 'lucide-react';
...
export default function AdminStorageTab() {
  ...
  const fetchFiles = async (path) => {
    ...
    try {
      ...
      if (err.code === 'storage/retry-limit-exceeded' || err.code === 'storage/unauthorized') {
        setError("Permission denied: Cannot access Firebase Storage. Please check your storage.rules to ensure you have 'list' and 'read' permissions.");
      } else {
  ...
  return (
    <div style={{ padding: '0.5rem', display: 'flex', flexDirection: 'column', gap: '1.5rem', maxWidth: '1000px', margin: '0 auto', position: 'relative' }}>
      ...
      {/* Breadcrumbs */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '1rem', background: 'white', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
      ...
      {/* File Explorer */}
      <div style={{ background: 'white', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', boxShadow: 'var(--shadow-sm)', overflow: 'hidden' }}>
        {loading ? (
          ...
          <div>
            ...
            {folders.map((folderRef, idx) => (
              ...
              >
                <Folder size={20} style={{ color: '#FCD34D', marginRight: '1rem', fill: '#FCD34D' }} />
                <span style={{ fontSize: '0.95rem', fontWeight: 600, color: 'var(--text-main)' }}>{folderRef.name}</span>
              </div>
            ))}
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center' }}>
                  {fileRef.name.toLowerCase().endsWith('.pdf') ? (
                    ...
                    <FileIcon size={20} style={{ color: 'var(--text-muted)', marginRight: '1rem' }} />
                  )}
                  <span style={{ fontSize: '0.9rem', color: 'var(--text-main)' }}>{fileRef.name}</span>
                </div>
                
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <button 
                    onClick={() => handleDownload(fileRef)}
                    style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', padding: '0.4rem', borderRadius: '4px' }}
                    title="Download"
                  >
                    <Download size={16} />
                  </button>
                  <button 
                    ...
                    style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--error)', padding: '0.4rem', borderRadius: '4px' }}
                    title="Delete"
                  >
                    <Trash2 size={16} />
                  </button>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: 'var(--surface)', padding: '2rem', borderRadius: 'var(--radius-md)', width: '90%', maxWidth: '400px', boxShadow: 'var(--shadow-lg)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <h3 style={{ margin: 0, color: 'var(--text-main)', fontSize: '1.1rem', fontWeight: 700 }}>New Folder</h3>
            ...
            <form onSubmit={handleCreateFolder}>
              <input 
                ...
                autoFocus
                style={{ width: '100%', padding: '0.75rem', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)', background: 'var(--bg-light)', color: 'var(--text-main)', fontSize: '0.95rem', marginBottom: '1.5rem' }}
              />
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem' }}>
                <button type="button" onClick={() => setShowFolderDialog(false)} style={{ padding: '0.6rem 1.2rem', borderRadius: 'var(--radius-sm)', border: 'none', background: 'transparent', color: 'var(--text-muted)', fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
                <button type="submit" disabled={!newFolderName.trim()} style={{ padding: '0.6rem 1.2rem', borderRadius: 'var(--radius-sm)', border: 'none', background: 'var(--primary)', color: 'white', fontWeight: 600, cursor: newFolderName.trim() ? 'pointer' : 'not-allowed', opacity: newFolderName.trim() ? 1 : 0.5 }}>Create</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {confirmDelete && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: 'var(--surface)', padding: '2rem', borderRadius: 'var(--radius-md)', width: '90%', maxWidth: '400px', boxShadow: 'var(--shadow-lg)' }}>
            <h3 style={{ margin: '0 0 1rem 0', color: 'var(--text-main)', fontSize: '1.1rem', fontWeight: 700 }}>Confirm Deletion</h3>
            <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginBottom: '1.5rem', lineHeight: 1.5 }}>
              Are you sure you want to permanently delete <strong>{confirmDelete.name}</strong>? This action cannot be undone.
            </p>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem' }}>
              <button onClick={() => setConfirmDelete(null)} style={{ padding: '0.6rem 1.2rem', borderRadius: 'var(--radius-sm)', border: 'none', background: 'transparent', color: 'var(--text-muted)', fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
              <button onClick={executeDelete} style={{ padding: '0.6rem 1.2rem', borderRadius: 'var(--radius-sm)', border: 'none', background: '#ef4444', color: 'white', fontWeight: 600, cursor: 'pointer' }}>Delete File</button>
            </div>
          </div>
        </div>
      )}
