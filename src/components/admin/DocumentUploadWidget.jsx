 ...
 *
 * A reusable, parametric upload widget for admin area.
 * Props allow configuration of accepted file types, UI labels, and default document type metadata.
 */
export default function DocumentUploadWidget({
  ...
  defaultDocumentType = 'UNKNOWN', // e.g., 'COA', 'PRICING_LIST'
  onUploadComplete, // optional callback receives the newly created Firestore doc
  extraMetadata = {}, // extra fields to store in Firestore alongside the file metadata
}) {
  const { user } = useAuth();
  ...
  // Listen to uploaded_documents collection
  useEffect(() => {
    const q = query(collection(db, 'uploaded_documents'), orderBy('createdAt', 'desc'));
    ...
    uploadTask.on(
      ...
      async () => {
        try {
          ...
          const docRef = await addDoc(collection(db, 'uploaded_documents'), {
            fileName: file.name,
            fileType: file.type,
            fileSize: file.size,
            url: downloadURL,
