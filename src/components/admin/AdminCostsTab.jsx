...
export default function AdminCostsTab({ readOnly = false }) {
  ...
  const filteredProducts = products.filter(p => {
    ...
    return (p.name || '').toLowerCase().includes(term) || (p.dosage || '').toLowerCase().includes(term);
  });
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
          ...
          {readOnly ? (
            ...
            <input 
              ...
              onKeyDown={(e) => { if (e.key === 'Enter') handleUpdateProduct(p.id, { costPrice: parseFloat(e.target.value) || 0 }); }}
              style={{ width: '100px', padding: '0.5rem', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)', textAlign: 'center' }}
            />
          )}
        </div>
      )
  ...
  if (loading) return <div style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-muted)' }}>Loading cost data...</div>;
  ...
  return (
    <div className="view-container">
      ...
      <div style={{ textAlign: 'right', fontSize: '10px', color: 'var(--text-muted)', opacity: 0.5, marginTop: '0.5rem' }}>
