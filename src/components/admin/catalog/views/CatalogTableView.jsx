...
export default function CatalogTableView({
  ...
  const columns = useMemo(() => {
      ...
      },
      {
        ...
        render: (row) => (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
            <div style={{ fontWeight: 600, color: 'var(--text-main)', fontSize: '0.9rem' }}>{row.name || row.displayName}</div>
    ...
    if (contextualTab === 'general') {
      contextColumns = [
        { key: 'category', header: 'Category', render: row => <span style={{ fontSize: '0.85rem' }}>{row.category || '-'}</span> },
        { key: 'format', header: 'Format', render: row => <span style={{ fontSize: '0.85rem' }}>{row.format || '-'}</span> },
        { key: 'size', header: 'Size', render: row => <span style={{ fontSize: '0.85rem' }}>{row.size || '-'}</span> },
        {
          key: 'coverage',
          header: 'Coverage',
          render: (row) => {
            ...
            return (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', alignItems: 'flex-start' }}>
                <div style={{ fontWeight: 600, color: color, fontSize: '0.75rem', background: bg, padding: '2px 8px', borderRadius: '12px' }}>
      ...
      contextColumns = [
        { key: 'supplier', header: 'Supplier', render: row => <span style={{ fontSize: '0.85rem', color: row.supplier ? '#1e293b' : '#94a3b8' }}>{row.supplier || 'Unassigned'}</span> },
        { key: 'cost', header: 'Unit Cost', render: row => <span style={{ fontSize: '0.85rem' }}>{row.cost ? `$${row.cost}` : '-'}</span> },
        { key: 'wholesale', header: 'Wholesale', render: row => <span style={{ fontSize: '0.85rem' }}>{row.wholesalePrice ? `$${row.wholesalePrice}` : '-'}</span> },
        { key: 'clinic', header: 'Clinic', render: row => <span style={{ fontSize: '0.85rem' }}>{row.clinicPrice ? `$${row.clinicPrice}` : '-'}</span> },
        { key: 'msrp', header: 'MSRP', render: row => <span style={{ fontSize: '0.85rem', fontWeight: 600 }}>{row.msrp ? `$${row.msrp}` : '-'}</span> },
        { 
          key: 'margin', 
          header: 'Margin', 
          render: row => {
            ...
            return <span style={{ fontSize: '0.85rem', color: margin > 50 ? '#10b981' : '#f59e0b' }}>{margin.toFixed(0)}%</span>;
          } 
        }
      ];
    } else if (contextualTab === 'costs') {
      contextColumns = [
        { key: 'supplier', header: 'Supplier', render: row => <span style={{ fontSize: '0.85rem' }}>{row.supplier || '-'}</span> },
        { key: 'cost', header: 'Base Cost', render: row => <span style={{ fontSize: '0.85rem' }}>{row.cost ? `$${row.cost}` : '-'}</span> },
        { key: 'shipping', header: 'Shipping/Duty', render: row => <span style={{ fontSize: '0.85rem' }}>{row.shippingCost ? `$${row.shippingCost}` : '-'}</span> },
        { key: 'landed', header: 'Landed Cost', render: row => <span style={{ fontSize: '0.85rem', fontWeight: 600 }}>{(row.cost && row.shippingCost) ? `$${(Number(row.cost) + Number(row.shippingCost)).toFixed(2)}` : '-'}</span> },
      ];
      ...
      contextColumns = [
        { 
          ...
          render: row => (
            <span style={{ fontSize: '0.85rem', fontWeight: 600, color: (row.stock || 0) < (row.reorderPoint || 20) ? '#ef4444' : '#1e293b' }}>
              {row.stock || 0} units
            </span>
          ) 
        },
        { key: 'reorderPoint', header: 'Reorder At', render: row => <span style={{ fontSize: '0.85rem' }}>{row.reorderPoint || 20}</span> },
        { key: 'moq', header: 'MOQ', render: row => <span style={{ fontSize: '0.85rem' }}>{row.moq || '-'}</span> },
        { key: 'leadTime', header: 'Lead Time', render: row => <span style={{ fontSize: '0.85rem' }}>{row.leadTime ? `${row.leadTime} days` : '-'}</span> },
        { key: 'velocity', header: 'Velocity', render: row => <span style={{ fontSize: '0.85rem' }}>{row.salesStatus || 'Medium'}</span> }
      ];
    } else if (contextualTab === 'regulatory') {
      contextColumns = [
        { key: 'gmp', header: 'GMP Status', render: row => <span style={{ fontSize: '0.85rem' }}>{row.gmp ? 'Valid' : 'Missing'}</span> },
        { key: 'coa', header: 'COA Status', render: row => <span style={{ fontSize: '0.85rem' }}>{row.coa ? 'Valid' : 'Missing'}</span> },
        { key: 'permit', header: 'Import Permit', render: row => <span style={{ fontSize: '0.85rem' }}>{row.importPermit || 'N/A'}</span> },
      ];
    }
      ...
      render: (row) => {
        ...
        return (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '0.5rem' }}>
            <button 
              onClick={(e) => { e.stopPropagation(); onAction('ai', productRef); }}
  ...
  useEffect(() => {
    if (user?.uid) {
      getDoc(doc(db, `users/${user.uid}/views/catalogProducts_${contextualTab}`)).then(snapshot => {
        if (snapshot.exists()) {
  ...
  const handleColumnToggle = async (key, isVisible) => {
    ...
    if (user?.uid) {
      await setDoc(doc(db, `users/${user.uid}/views/catalogProducts_${contextualTab}`), { visibleColumns: newCols }, { merge: true });
    }
  };
