...
export default function RegulatoryTrackerView({ variants = [] }) {
  const actualData = React.useMemo(() => {
    if (!variants || variants.length === 0) return mockProducts;
    return variants.map((v) => ({
      ...v,
      registration: v.registration || 'Active',
      coa: v.coa || 'Missing',
      gmp: v.gmp || 'Missing',
      stability: v.stability || 'Valid',
      permit: v.permit || 'Active',
      risk: v.coa === 'Missing' || v.gmp === 'Missing' ? 'Red' : 'Green',
    }));
  }, [variants]);
  ...
  const metrics = {
    ...
    missingStability: actualData.filter(
      (p) => p.stability === 'Missing' || p.stability === 'Expired'
    ).length,
    score: Math.max(
      0,
      100 -
        actualData.filter((p) => p.coa === 'Missing').length * 5 -
        actualData.filter((p) => p.gmp === 'Missing').length * 5
    ),
  };
    ...
    switch (risk) {
      case 'Green':
        return (
          ...
          >
            <CheckCircle2 size={12} style={{ marginRight: '4px' }} /> Low Risk
          </span>
        );
      case 'Amber':
        return (
          ...
          >
            <AlertTriangle size={12} style={{ marginRight: '4px' }} /> Med Risk
          </span>
        );
      case 'Red':
        return (
          ...
          >
            <XCircle size={12} style={{ marginRight: '4px' }} /> High Risk
          </span>
        );
      default:
        return null;
  ...
  const renderStatusIcon = (status) => {
    if (status === 'Valid' || status === 'Active')
      return <CheckCircle2 size={16} color="#10b981" style={{ margin: '0 auto' }} />;
    if (status === 'Missing')
      return <XCircle size={16} color="#f43f5e" style={{ margin: '0 auto' }} />;
    if (status === 'Pending')
      return <Clock size={16} color="#f59e0b" style={{ margin: '0 auto' }} />;
    if (status === 'Expired')
      return <AlertTriangle size={16} color="#f43f5e" style={{ margin: '0 auto' }} />;
    return <span style={{ fontSize: '0.875rem', color: '#6b7280' }}>{status}</span>;
  };

  return (
    <div
      className="regulatory-tracker"
      style={{
        display: 'flex',
    ...
    >
      ...
      >
        <div>
          ...
          >
            Monitor product registrations, certifications, and compliance risks.
          </p>
        </div>
      ...
      >
        {[
          ...
          },
          {
            ...
            value: metrics.missingStability,
            icon: TrendingUp,
            color: '#d97706',
            bg: '#fef3c7',
          },
              ...
              >
                <div>
                  ...
                  <p style={{ color: '#c7d2fe', fontSize: '0.875rem', margin: '4px 0 0 0' }}>
                    Ask me about regulatory statuses, missing documents, or risk profiles.
                  </p>
                </div>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', paddingTop: '4px' }}>
                  ...
                  <button
                    onClick={() => setAiQuery('Show suppliers with missing GMP')}
                    style={{
                      fontSize: '0.75rem',
                      padding: '4px 10px',
                      borderRadius: '9999px',
                  ...
                  <button
                    onClick={() => setAiQuery('Which products are at regulatory risk?')}
                    style={{
                      fontSize: '0.75rem',
                      padding: '4px 10px',
                      borderRadius: '9999px',
              ...
              >
                ...
                >
                  <tr>
                    ...
                    <th style={{ padding: '12px 12px', fontWeight: 500, textAlign: 'center' }}>
                      Registration
                    </th>
                    <th style={{ padding: '12px 12px', fontWeight: 500, textAlign: 'center' }}>
                      COA
                ...
                <tbody>
                  {actualData.map((product) => (
                    <tr key={product.id} style={{ borderBottom: '1px solid #e5e7eb' }}>
                      ...
                      <td style={{ padding: '12px', whiteSpace: 'nowrap', textAlign: 'center' }}>
                        {renderStatusIcon(product.permit)}
                      </td>
                      <td
                        style={{ padding: '12px 16px', whiteSpace: 'nowrap', textAlign: 'right' }}
                      >
                  ...
                  <div key={product.id} className="mobile-card">
                    ...
                    >
                      ...
                      >
                        ...
                        <span>{renderStatusIcon(product.registration)}</span>
                      </div>
                      <div
                        style={{
                          display: 'flex',
                      ...
                      >
                        <span style={{ color: '#6b7280' }}>COA:</span>{' '}
                        <span>{renderStatusIcon(product.coa)}</span>
                      </div>
                      <div
                        style={{
                          display: 'flex',
                      ...
                      >
                        <span style={{ color: '#6b7280' }}>GMP:</span>{' '}
                        <span>{renderStatusIcon(product.gmp)}</span>
                      </div>
                      <div
                        style={{
                          display: 'flex',
                      ...
                      >
                        ...
                        <span>{renderStatusIcon(product.stability)}</span>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          >
            ...
            <div style={{ padding: '8px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
              ...
              >
                ...
                <div>
                  <h4
                    style={{ fontSize: '0.875rem', fontWeight: 500, color: '#111827', margin: 0 }}
                  >
                    Registration expiring soon
                  </h4>
                  <p style={{ fontSize: '0.75rem', color: '#6b7280', margin: '2px 0 0 0' }}>
                    <span style={{ fontWeight: 600 }}>Ipamorelin 5mg</span> expires in 12 days.
                  </p>
                  <button
                    style={{
                      fontSize: '0.75rem',
              ...
              >
                ...
                <div>
                  <h4
                    style={{ fontSize: '0.875rem', fontWeight: 500, color: '#111827', margin: 0 }}
                  >
                    Missing GMP Certificate
                  </h4>
                  <p style={{ fontSize: '0.75rem', color: '#6b7280', margin: '2px 0 0 0' }}>
                    <span style={{ fontWeight: 600 }}>BioPeptide Labs</span> is missing current GMP.
                  </p>
                  <button
                    style={{
                      fontSize: '0.75rem',
              ...
              >
                ...
                <div>
                  <h4
                    style={{ fontSize: '0.875rem', fontWeight: 500, color: '#111827', margin: 0 }}
                  >
                    Missing COA (Batch #492)
                  </h4>
                  <p style={{ fontSize: '0.75rem', color: '#6b7280', margin: '2px 0 0 0' }}>
                    <span style={{ fontWeight: 600 }}>TB-500 10mg</span> shipment arrived without
                    COA.
                  </p>
                  <button
                    style={{
            ...
            <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              >
                ...
                <div style={{ fontSize: '0.875rem', fontWeight: 500, color: '#111827' }}>
                  EuroPeptides GMP Renewal
                </div>
                <div style={{ fontSize: '0.75rem', color: '#6b7280', marginTop: '4px' }}>
                  Supplier facility certification renewal
                </div>
              </div>
              <div
                style={{
              ...
              >
                ...
                <div style={{ fontSize: '0.875rem', fontWeight: 500, color: '#111827' }}>
                  Ipamorelin Registration Expiry
                </div>
                <div style={{ fontSize: '0.75rem', color: '#6b7280', marginTop: '4px' }}>
                  Requires updated stability data submission
                </div>
              </div>
              <div
                style={{
              ...
              >
                ...
                <div style={{ fontSize: '0.75rem', color: '#6b7280', marginTop: '4px' }}>
                  Quarterly compliance document review
                </div>
              </div>
            </div>
            <div
