...
const MOCK_SUPPLIERS = [
  {
    ...
    recommendation: 'Top performing supplier. Consider consolidating more volume here for tier discounts.',
...
export default function SupplierInsightsView({ products }) {
  ...
  const getStatusBadge = (status) => {
    switch(status) {
      case 'active': return 'si-badge si-badge-success';
      case 'at-risk': return 'si-badge si-badge-warning';
      case 'critical': return 'si-badge si-badge-error';
      default: return 'si-badge';
    }
  };

  return (
    <div className="supplier-insights" style={{ padding: '2rem', maxWidth: '1400px', margin: '0 auto', color: 'var(--text-main, #1f2937)' }}>
      <style>
      {`
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-muted, #6b7280)', marginTop: '0.5rem', fontSize: '1.1rem' }}>
            AI-driven analytics and risk management for your supply chain.
          </p>
        </div>
        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
          <div className="si-glass" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem', borderRadius: '999px' }}>
            ...
            <input 
              ...
              style={{ border: 'none', background: 'transparent', outline: 'none', color: 'var(--text-main, #111827)', width: '200px' }}
            />
          </div>
          <button className="si-btn-icon" style={{ background: 'var(--bg-card, #fff)', border: '1px solid var(--border, #e5e7eb)' }}>
            <Filter size={20} />
          </button>
        </div>
      </div>
      ...
      <div className="si-grid si-stats" style={{ marginBottom: '2rem' }}>
        {MOCK_STATS.map((stat, idx) => (
          ...
          >
            ...
            <div>
              <div style={{ color: 'var(--text-muted, #6b7280)', fontSize: '0.875rem', fontWeight: '500' }}>{stat.label}</div>
              <div style={{ fontSize: '1.5rem', fontWeight: '700', color: 'var(--text-main, #111827)' }}>{stat.value}</div>
            </div>
          </motion.div>
        ))}
      </div>
      ...
      <div className="si-grid si-2col" style={{ marginBottom: '2rem' }}>
        <motion.div className="si-card" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
          <h3 style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: 0, fontSize: '1.25rem' }}>
            <AlertTriangle color="#ef4444" size={24} />
            Supplier Risk Center
          </h3>
          <div style={{ marginTop: '1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {suppliers.flatMap(s => s.alerts.map((alert, i) => (
              <div key={`${s.id}-${i}`} style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem', padding: '1rem', background: 'rgba(239, 68, 68, 0.05)', borderRadius: '12px', borderLeft: '4px solid #ef4444' }}>
                <AlertCircle size={20} color="#ef4444" style={{ flexShrink: 0, marginTop: '0.1rem' }} />
                <div>
                  <strong style={{ display: 'block', fontSize: '0.9rem', color: '#991b1b' }}>{s.name}</strong>
        ...
        <motion.div className="si-card si-ai-card" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
          <h3 style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: 0, fontSize: '1.25rem', color: '#6d28d9' }}>
            <Sparkles color="#8b5cf6" size={24} />
            AI Insights & Recommendations
          </h3>
          <div style={{ marginTop: '1rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {suppliers.map(s => (
              <div key={`ai-${s.id}`} style={{ padding: '1rem', background: 'rgba(255,255,255,0.6)', borderRadius: '12px', border: '1px solid rgba(139, 92, 246, 0.1)' }}>
                ...
                <p style={{ margin: 0, fontSize: '0.875rem', color: 'var(--text-muted, #4b5563)', lineHeight: 1.5 }}>
                  {s.recommendation}
                </p>
              </div>
            )).slice(0,3)}
            <div style={{ marginTop: '1rem' }}>
              <div style={{ fontSize: '0.8rem', textTransform: 'uppercase', color: '#8b5cf6', fontWeight: 600, marginBottom: '0.5rem', letterSpacing: '0.05em' }}>Contextual Actions</div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                <div className="si-action-chip"><Search size={14}/> Find backup suppliers</div>
                <div className="si-action-chip"><TrendingUp size={14}/> Analyze pricing trends</div>
                <div className="si-action-chip"><ShieldAlert size={14}/> Generate risk report</div>
              </div>
            </div>
          </div>
        </motion.div>
      ...
      <motion.div className="si-card" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="si-mobile-card-container">
          {suppliers.map(s => (
            <div key={`mob-${s.id}`} className="si-mobile-card" onClick={() => setSelectedSupplier(s)}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                ...
                <div className="si-score-ring" style={{ border: `4px solid ${getHealthColor(s.healthScore)}`, color: getHealthColor(s.healthScore) }}>
                  {s.healthScore}
                </div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', fontSize: '0.875rem', color: 'var(--text-muted)' }}>
                <div><strong>Lead Time:</strong> {s.avgLeadTime}d</div>
                <div><strong>Reliability:</strong> {s.reliability}%</div>
                <div><strong>Products:</strong> {s.productsSupplied}</div>
                <div><strong>Quality:</strong> {s.qualityScore}/100</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <AnimatePresence>
        {selectedSupplier && (
          <>
            ...
            >
              <div style={{ padding: '2rem', borderBottom: '1px solid var(--border, #e5e7eb)', background: 'var(--bg-card, #fff)', position: 'sticky', top: 0, zIndex: 10 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    <h2 style={{ margin: '0 0 0.5rem 0', fontSize: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      {selectedSupplier.name}
                      <span className={getStatusBadge(selectedSupplier.status)} style={{ marginLeft: '0.5rem' }}>{selectedSupplier.status.replace('-', ' ')}</span>
                    </h2>
              ...
              <div style={{ padding: '2rem', flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', marginBottom: '2rem', background: 'var(--bg-hover, #f9fafb)', padding: '1.5rem', borderRadius: '12px' }}>
                  <div className="si-score-ring" style={{ width: '80px', height: '80px', fontSize: '2rem', border: `6px solid ${getHealthColor(selectedSupplier.healthScore)}`, color: getHealthColor(selectedSupplier.healthScore) }}>
                    {selectedSupplier.healthScore}
                  </div>
                  <div>
                    <h3 style={{ margin: '0 0 0.25rem 0' }}>Overall Health</h3>
                    <p style={{ margin: 0, fontSize: '0.875rem', color: 'var(--text-muted)' }}>Based on delivery, quality, and pricing metrics.</p>
                  </div>
                </div>

                <div className="si-ai-card" style={{ padding: '1.5rem', borderRadius: '12px', marginBottom: '2rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem', color: '#6d28d9', fontWeight: 600 }}>
                    <Sparkles size={18} /> AI Analysis
                  </div>
                  <p style={{ margin: 0, fontSize: '0.9rem', lineHeight: 1.5 }}>{selectedSupplier.recommendation}</p>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '2rem' }}>
                  <div style={{ border: '1px solid var(--border, #e5e7eb)', padding: '1rem', borderRadius: '12px' }}>
                    <div style={{ color: 'var(--text-muted)', fontSize: '0.8rem', marginBottom: '0.25rem' }}>Products Supplied</div>
                    <div style={{ fontSize: '1.25rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Package size={18}/> {selectedSupplier.productsSupplied}</div>
                  </div>
                  <div style={{ border: '1px solid var(--border, #e5e7eb)', padding: '1rem', borderRadius: '12px' }}>
                    <div style={{ color: 'var(--text-muted)', fontSize: '0.8rem', marginBottom: '0.25rem' }}>Total Spend</div>
                    <div style={{ fontSize: '1.25rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.5rem' }}><DollarSign size={18}/> {selectedSupplier.totalSpent}</div>
                  </div>
                  <div style={{ border: '1px solid var(--border, #e5e7eb)', padding: '1rem', borderRadius: '12px' }}>
                    <div style={{ color: 'var(--text-muted)', fontSize: '0.8rem', marginBottom: '0.25rem' }}>Recent POs</div>
                    <div style={{ fontSize: '1.25rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.5rem' }}><FileText size={18}/> {selectedSupplier.recentPOs}</div>
                  </div>
                  <div style={{ border: '1px solid var(--border, #e5e7eb)', padding: '1rem', borderRadius: '12px' }}>
                    <div style={{ color: 'var(--text-muted)', fontSize: '0.8rem', marginBottom: '0.25rem' }}>Compliance</div>
                    <div style={{ fontSize: '0.9rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: '0.25rem' }}><ShieldCheck size={18} color="#10b981"/> {selectedSupplier.compliance}</div>
                  </div>
                </div>

                <h4 style={{ borderBottom: '1px solid var(--border, #e5e7eb)', paddingBottom: '0.5rem', marginBottom: '1rem' }}>Performance Metrics</h4>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '2rem' }}>
                  <div>
                    ...
                    <div style={{ height: '8px', background: 'var(--border, #e5e7eb)', borderRadius: '4px', overflow: 'hidden' }}>
                      <div style={{ width: `${selectedSupplier.qualityScore}%`, height: '100%', background: '#3b82f6', borderRadius: '4px' }}></div>
                    </div>
                  </div>
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.875rem', marginBottom: '0.25rem' }}>
                    ...
                    <div style={{ height: '8px', background: 'var(--border, #e5e7eb)', borderRadius: '4px', overflow: 'hidden' }}>
                      <div style={{ width: `${selectedSupplier.reliability}%`, height: '100%', background: '#10b981', borderRadius: '4px' }}></div>
                    </div>
                  </div>
                </div>

                <h4 style={{ borderBottom: '1px solid var(--border, #e5e7eb)', paddingBottom: '0.5rem', marginBottom: '1rem' }}>Pricing & Health Trend</h4>
                <div style={{ height: '150px', display: 'flex', alignItems: 'flex-end', gap: '0.5rem', padding: '1rem', background: 'var(--bg-hover, #f9fafb)', borderRadius: '12px' }}>
                  {selectedSupplier.trendData.map((val, i) => (
                    <div key={i} style={{ flex: 1, background: '#8b5cf6', height: `${val}%`, borderRadius: '4px 4px 0 0', opacity: 0.8 }}></div>
                  ))}
                </div>
