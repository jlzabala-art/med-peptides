...
export default function SmartProductIntakeWizard({
  isOpen,
  onClose,
  onAddProduct,
  mode = 'product',
  ...
  const handleAiUpload = async (e) => {
    ...
    try {
      ...
      if (items && items.length > 0) {
        ...
        if (mode === 'variant') {
          // Try to auto-match a parent product based on name
          if (first.peptideName) {
            const match = products.find(
              (p) => p.name.toLowerCase() === first.peptideName.toLowerCase()
            );
            if (match) {
              setFormData((prev) => ({ ...prev, parentProductId: match.id }));
            }
          }
        }
  ...
  const handleSave = async () => {
    if (mode === 'variant' && !formData.parentProductId) {
      toast.error('Please select a parent product.');
      return;
    }
    if (mode === 'product' && !formData.name) {
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {mode === 'variant' ? 'New Variant' : 'New Product'}
            </span>
          </h2>
          <button
            onClick={onClose}
        ...
        <div style={{ padding: '1.5rem', overflowY: 'auto', flex: 1 }}>
          ...
          {step === 3 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              >
                <h4 style={{ margin: 0, fontSize: '0.875rem', fontWeight: 600, color: '#92400e' }}>
                ...
                >
                  <input
                    type="checkbox"
                    checked={formData.missingCOA}
                    onChange={(e) => setFormData({ ...formData, missingCOA: e.target.checked })}
                  />
                ...
                >
                  <input
                    type="checkbox"
                    checked={formData.missingSDS}
                    onChange={(e) => setFormData({ ...formData, missingSDS: e.target.checked })}
                  />
                  Flag as Missing SDS
                </label>
              </div>
        ...
        >
          ...
          {step < 3 ? (
            ...
            >
              <Save size={16} /> Save {mode === 'variant' ? 'Variant' : 'Product'}
            </button>
          )}
        </div>
      </div>
