...
export const PRESET_VIEWS = [
  ...
  },
  {
    ...
    filters: [
      { id: 'dateRange', value: 'last7days', label: 'Imported Last 7 Days' },
      { id: 'missingData', value: 'true', label: 'Missing Data' },
    ],
    sortConfig: [{ key: 'importDate', direction: 'desc' }],
  },
  {
...
export function useCatalogFilters(items = []) {
  ...
  const handleActiveViewChange = async (viewId) => {
    ...
    if (user?.uid) {
      await setDoc(
        doc(db, `users/${user.uid}/catalogViews/custom`),
        { activeViewId: viewId },
        { merge: true }
      );
    }
    ...
    if (filters.length > 0) {
      result = result.filter((item) => {
        return filters.every((f) => {
          ...
          if (f.id === 'missingData') {
            return (
              item.isMissingSupplier ||
              item.isMissingPricing ||
              item.isMissingImages ||
              item.coa === 'Missing'
            );
          }
          if (f.id === 'missingCoa') {
