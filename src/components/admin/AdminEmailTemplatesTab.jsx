...
export default function AdminEmailTemplatesTab() {
  ...
  const previewHtml = useMemo(() => {
    if (!selected) return '';
    try { return selected.getHtml(); }
    catch (e) { return `<p style="color:red">Preview error: ${e.message}</p>`; }
  }, [selected]);
  ...
  const filtered = EMAIL_TEMPLATE_REGISTRY.filter(t => {
    ...
    const matchesSearch = !search || t.name.toLowerCase().includes(search.toLowerCase()) || t.id.includes(search.toUpperCase());
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h2 style={{ margin: '0 0 4px', fontSize: '1.25rem', fontWeight: 700, color: 'var(--primary)' }}>
        ...
        <input
          ...
          style={{
            ...
            width: '220px', color: 'var(--text-main)', background: 'var(--background)',
          }}
        />
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
        {FILTERS.map(f => (
          <button
            ...
            style={{
              padding: '0.35rem 0.875rem', borderRadius: 'var(--radius-md)', fontSize: '0.8rem', fontWeight: 600,
          ...
          {filtered.map((tpl, idx) => (
            <div
              ...
              style={{
                ...
                borderLeft: selectedId === tpl.id ? '3px solid var(--primary)' : '3px solid transparent',
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.35rem' }}>
                ...
                <button
                  onClick={e => { e.stopPropagation(); copyId(tpl.id); }}
                  title="Copy ID"
                  style={{
              ...
              <div style={{ fontWeight: 600, fontSize: '0.875rem', color: 'var(--text-main)', marginBottom: '0.3rem', lineHeight: 1.3 }}>
                {tpl.name}
              </div>
        ...
        {selected ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            <div className="card" style={{ padding: '1.25rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '1rem' }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '4px' }}>
                    <span style={{
                      fontFamily: 'monospace', fontSize: '0.875rem', fontWeight: 800,
                      backgroundColor: 'var(--color-primary)', color: 'white',
                      padding: '3px 10px', borderRadius: '5px', letterSpacing: '0.5px',
                    }}>
                      {selected.id}
                    </span>
                    <button
                      ...
                      style={{
                        ...
                        cursor: 'pointer', fontSize: '0.78rem', color: copiedId === selected.id ? 'var(--success)' : 'var(--text-muted)',
                      }}
                    >
                      {copiedId === selected.id ? <><Check size={12} /> Copied!</> : <><Copy size={12} /> Copy ID</>}
                    </button>
                  </div>
                  <h3 style={{ margin: 0, fontSize: '1.05rem', color: 'var(--text-main)' }}>{selected.name}</h3>
                </div>
              ...
              <p style={{ margin: '0 0 1rem', fontSize: '0.875rem', color: 'var(--text-muted)', lineHeight: 1.6 }}>
                {selected.description}
              </p>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                <div style={{ background: 'var(--background)', borderRadius: 'var(--radius-sm)', padding: '0.75rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '4px', color: 'var(--text-muted)', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                    <Zap size={12} /> Trigger
                  </div>
                  <div style={{ fontSize: '0.8rem', color: 'var(--text-main)', fontFamily: 'monospace', lineHeight: 1.5 }}>
                    {selected.trigger}
                  </div>
                </div>
                <div style={{ background: 'var(--background)', borderRadius: 'var(--radius-sm)', padding: '0.75rem' }}>
                  ...
                  <div style={{ fontSize: '0.78rem', color: 'var(--primary)', fontFamily: 'monospace', lineHeight: 1.5, wordBreak: 'break-all' }}>
                    {selected.sourceFile}
                  </div>
                </div>
              </div>

              <div style={{ marginTop: '0.75rem', background: 'var(--background)', borderRadius: 'var(--radius-sm)', padding: '0.75rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '4px', color: 'var(--text-muted)', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                  <Mail size={12} /> Channel
                </div>
                <div style={{ fontSize: '0.8rem', color: 'var(--text-main)' }}>{selected.channel}</div>
              </div>
            </div>
            ...
            <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
              ...
              <div style={{ backgroundColor: '#f1f5f9', padding: '1.5rem', display: 'flex', justifyContent: 'center' }}>
                <div
                  style={{
                    backgroundColor: 'white', width: '100%', maxWidth: '620px',
