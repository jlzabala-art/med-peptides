...
import {
  Layers, ClipboardList, Plus, Minus, Trash2, Send, Save,
  ChevronDown, ChevronUp, ShoppingBag, User, Building,
  CheckCircle2, AlertCircle, Loader2, RefreshCw, Eye, Search, ArrowLeft
} from 'lucide-react';
import {
  BULK_STATUS, RX_STATUS, ITEM_UNITS
  ...
  return (
    <div style={{ position: 'relative' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem',
        ...
        {loading ? <Loader2 size={14} color="var(--color-text-tertiary)" style={{ animation: 'adminSpin 1s linear infinite' }} />
        ...
        <input value={q} onChange={e => handleInput(e.target.value)}
          ...
          style={{ flex: 1, border: 'none', background: 'none', outline: 'none',
            fontSize: '0.78rem', color: 'var(--color-text-primary)', fontFamily: 'inherit' }} />
      </div>

      {results.length > 0 && (
        <div style={{
          position: 'absolute', top: 'calc(100% + 4px)', left: 0, right: 0, zIndex: 100,
          background: 'var(--color-bg-surface)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)',
  ...
  return (
    <div 
      ...
      style={{
        ...
        background: alreadyInBulk ? 'var(--color-bg-app)' : (selected ? 'rgba(0,54,102,0.01)' : 'var(--color-bg-surface)'),
      ...
      <div style={{ padding: '0.6rem 0.75rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
        ...
        <div style={{ width: 28, height: 28, borderRadius: '6px', flexShrink: 0,
          ...
          display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {isPrescription ? <ClipboardList size={12} color="var(--color-primary)" /> : <ShoppingBag size={12} color="var(--color-success)" />}
        </div>

        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 800, fontSize: '0.78rem', color: 'var(--color-text-primary)',
            overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {isPrescription
              ? (item.patient?.name || item.patient?.email || 'Paciente sin nombre')
              : (item.customerName || item.userName || item.uid?.slice(0, 8))}
          </div>
          <div style={{ fontSize: '0.62rem', color: 'var(--color-text-tertiary)', fontWeight: 600, marginTop: '0.1rem',
      ...
      {open && (
        <div style={{ padding: '0.5rem 0.75rem', borderTop: '1px solid #f8fafc', background: '#fafbfc' }}>
...
function CatalogItemRow({ item, index, onChange, onRemove }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem',
      padding: '0.5rem 0.75rem', borderRadius: '6px', background: 'var(--color-bg-app)',
      border: '1px solid #f1f5f9' }}>
      <span style={{ fontSize: '0.8rem' }}>{item.type === 'protocol' ? '🧬' : '💊'}</span>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: '0.78rem', fontWeight: 800, color: 'var(--color-text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {item.name}
        </div>
        {item.sku && <div style={{ fontSize: '0.62rem', color: 'var(--color-text-tertiary)' }}>SKU: {item.sku}</div>}
      </div>
...
const qb = { width: 20, height: 20, borderRadius: '4px', border: '1px solid #e2e8f0',
  background: 'var(--color-bg-surface)', cursor: 'pointer', fontSize: '0.78rem', fontWeight: 700, color: 'var(--color-text-secondary)',
  display: 'flex', alignItems: 'center', justifyContent: 'center' };
const inpSelect = { fontSize: '0.72rem', border: '1px solid #e2e8f0', borderRadius: '4px',
  padding: '0.2rem 0.3rem', outline: 'none', fontFamily: 'inherit', color: 'var(--color-text-primary)', backgroundColor: 'var(--color-bg-surface)' };

// ── Aggregated preview item ───────────────────────────────────────────────────
function AggregatedItemRow({ item }) {
  const [open, setOpen] = useState(false);
  return (
    <div style={{ padding: '0.5rem 0.75rem', borderRadius: '6px', background: 'var(--color-bg-app)',
      border: '1px solid #f1f5f9' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
        <span>{item.type === 'protocol' ? '🧬' : '💊'}</span>
        <span style={{ fontWeight: 800, fontSize: '0.78rem', color: 'var(--color-text-primary)', flex: 1 }}>{item.name || item.id}</span>
        <span style={{ fontWeight: 900, fontSize: '0.85rem', color: 'var(--color-primary)', minWidth: 24, textAlign: 'right' }}>{item.quantity}</span>
        <span style={{ fontSize: '0.65rem', color: 'var(--color-text-tertiary)', minWidth: 32 }}>{item.unit}</span>
        {item.sources?.length > 0 && (
          <button onClick={() => setOpen(v => !v)}
            style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-border)', padding: '0.1rem' }}>
            <Eye size={12} />
          </button>
        )}
      </div>
      {open && item.sources?.length > 0 && (
        <div style={{ marginTop: '0.3rem', paddingLeft: '1.2rem', display: 'flex', flexDirection: 'column', gap: '0.15rem' }}>
          {item.sources.map((s, i) => (
            <div key={i} style={{ fontSize: '0.62rem', color: 'var(--color-text-tertiary)', fontWeight: 600 }}>
              {s.type === 'own' ? '📦 stock catalog' : s.type === 'prescription' ? `💊 Rx: ${s.patientName || '—'} (Dr. ${s.doctorName || '—'})` : `🛒 Order: ${s.patientName || '—'}`}
...
export default function AdminBulkOrderBuilder({ onBack, onSuccess }) {
  ...
  // ── Load wholesalers ──
  useEffect(() => {
    getDocs(query(collection(db, 'users'), where('role', '==', 'wholesaler'), orderBy('firstName'), limit(100)))
      .then(snap => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const validRx = rxList.filter(d => [RX_STATUS.SENT, RX_STATUS.ASSIGNED_TO_WS].includes(d.status));
  ...
  const updateCatalogItem = (i, v) => setCatalogItems(p => p.map((x, idx) => idx === i ? v : x));
  const removeCatalogItem = (i) => setCatalogItems(p => p.filter((_, idx) => idx !== i));

  // ── Batch Create Bulk Order B2B ──
  const handleSubmit = async () => {
    if (!selectedWS) { setErrorMsg('Selecciona un Wholesaler.'); return; }
    if (aggregated.length === 0) { setErrorMsg('Añade al menos un producto o selecciona prescripciones/órdenes.'); return; }
    ...
    try {
      ...
      const wsName = `${selectedWS.firstName || ''} ${selectedWS.lastName || ''}`.trim() || 'Wholesaler';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      {errorMsg && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem',
          padding: '0.6rem 0.85rem', borderRadius: '4px',
      ...
      <div style={{ background: 'var(--color-bg-surface)', padding: '1rem', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)' }}>
        <label style={{ display: 'block', fontSize: '0.65rem', fontWeight: 900, color: 'var(--color-text-tertiary)',
        ...
        <select 
          ...
          style={{ width: '100%', padding: '0.55rem', borderRadius: '4px', border: '1px solid var(--border)', backgroundColor: 'var(--color-bg-surface)', fontSize: '0.82rem' }}
        >
      ...
      {selectedWS ? (
        <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '1.25rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            {/* Tabs Navigation */}
            <div style={{ display: 'flex', gap: '0.2rem', padding: '0.2rem', borderRadius: '6px', background: '#f1f5f9' }}>
              {[
                ...
                <button key={t.id} onClick={() => setTab(t.id)} style={{
                  flex: 1, padding: '0.45rem 0.35rem', borderRadius: '4px', cursor: 'pointer',
                  border: 'none', fontFamily: 'inherit', fontSize: '0.7rem', fontWeight: 700,
            ...
            {/* Tab content */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', minHeight: 280 }}>
              
              {loadingWSData ? (
                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: 200 }}>
                  <Loader2 size={24} style={{ animation: 'adminSpin 1s linear infinite' }} color="#6366f1" />
                </div>
              ) : (
                <>
                  {/* Prescriptions Tab */}
                  {tab === 'prescriptions' && (
                    prescriptions.length === 0 ? (
                      <div style={emptyTab}>No hay prescripciones activas asignadas a este wholesaler.</div>
                    ) : (
                  ...
                  {tab === 'orders' && (
                    orders.length === 0 ? (
                      <div style={emptyTab}>No hay órdenes B2C de pacientes sin pedido bulk asignado.</div>
                    ) : (
                      orders.map(o => (
                  ...
                  {tab === 'catalog' && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                      ...
                      {catalogItems.length === 0 ? (
                        <div style={emptyTab}>Busca productos o protocolos arriba para agregarlos directamente.</div>
                      ) : (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            <div style={{ background: 'var(--color-bg-surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: '0.65rem', fontWeight: 900, color: 'var(--color-text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.07em' }}>
                  Resumen Consolidado B2B
                </span>
                <span style={{ fontSize: '0.65rem', fontWeight: 800, background: 'rgba(99,102,241,0.06)', color: '#6366f1', padding: '0.15rem 0.4rem', borderRadius: '4px' }}>
                  {totalSelected} fuentes
                </span>
              </div>

              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem', maxHeight: 320, overflowY: 'auto', paddingRight: '0.2rem' }}>
                {aggregated.length === 0 ? (
                  <div style={{ textAlign: 'center', color: 'var(--color-text-tertiary)', fontSize: '0.75rem', padding: '2rem 0' }}>
                    Selecciona fuentes a la izquierda o añade ítems del catálogo para ver el resumen consolidado.
                  </div>
                ) : (
                  aggregated.map((item, i) => (
                    <AggregatedItemRow key={i} item={item} />
                  ))
                )}
              </div>

              {aggregated.length > 0 && (
                <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '0.5rem', display: 'flex', justifyContent: 'space-between', fontSize: '0.72rem', fontWeight: 700, color: 'var(--color-text-secondary)' }}>
                  <span>{aggregated.length} Líneas totales</span>
                  <span style={{ color: 'var(--color-primary)', fontSize: '0.78rem' }}>{aggregated.reduce((s, i) => s + i.quantity, 0)} unidades</span>
                </div>
              )}
            </div>

            {/* Notes */}
            <div style={{ background: 'var(--color-bg-surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', padding: '1rem' }}>
              <label style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem',
                fontSize: '0.65rem', fontWeight: 900, color: 'var(--color-text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.07em' }}>
                Notas de la orden
                <textarea 
                  value={notes} 
                  ...
                  rows={2}
                  style={{ fontSize: '0.78rem', border: '1px solid var(--border)', borderRadius: '4px', padding: '0.45rem', outline: 'none', fontFamily: 'inherit', color: 'var(--color-text-primary)', resize: 'vertical' }}
                />
              </label>
            </div>
            ...
            <button 
              ...
              style={{
                ...
                padding: '0.75rem', borderRadius: 'var(--radius-sm)', cursor: submitting || aggregated.length === 0 ? 'not-allowed' : 'pointer',
        ...
        <div style={{ textAlign: 'center', padding: '4rem', color: 'var(--color-text-tertiary)', border: '1.5px dashed #e2e8f0', borderRadius: '8px', background: '#fafbfc' }}>
          <Building size={32} style={{ opacity: 0.4, marginBottom: '0.5rem' }} />
          <div style={{ fontWeight: 700, fontSize: '0.85rem' }}>Selecciona un wholesaler arriba</div>
          <div style={{ fontSize: '0.72rem', marginTop: '0.2rem' }}>Podrás consolidar prescripciones, órdenes o agregar ítems propios para este canal.</div>
        </div>
      )}
...
const emptyTab = {
  textAlign: 'center', padding: '3rem 1rem', color: 'var(--color-text-tertiary)', fontSize: '0.75rem',
  border: '1px dashed #e2e8f0', borderRadius: '6px', background: '#fafbfc'
};
