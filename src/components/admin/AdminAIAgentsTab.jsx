...
import { Bot, RefreshCw, CheckCircle, XCircle, Clock, Zap, Globe, AlertTriangle, ExternalLink, ToggleLeft, ToggleRight, ChevronDown, ChevronUp } from 'lucide-react';
...
function StatusBadge({ status }) {
  const map = {
    ...
    disabled: { label: 'Disabled', color: 'var(--color-danger)', bg: 'rgba(239,68,68,0.1)',   Icon: XCircle     },
    error:    { label: 'Error',    color: 'var(--color-danger)', bg: 'rgba(239,68,68,0.1)',   Icon: AlertTriangle },
  };
  ...
  return (
    <span style={{
      ...
      background: bg, color, fontSize: '0.72rem', fontWeight: 800,
    }}>
      <Icon size={11} />
      {label}
    </span>
...
function ModelBadge({ model }) {
  ...
  return (
    <span style={{
      ...
      background: `${color}15`, color, fontSize: '0.7rem', fontWeight: 700,
...
function AgentCard({ agentKey, agent, onEdit, isActive, saving, onToggle }) {
  ...
  return (
    <div style={{
      background: 'var(--surface)', border: `1px solid ${isActive ? 'var(--success)' : 'var(--border)'}`,
      ...
      {/* Card Header */}
      <div style={{ 
        padding: '1.25rem', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '1rem',
        ...
        <div style={{ display: 'flex', gap: '1rem', flex: 1, minWidth: 0 }}>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.2rem', flexWrap: 'wrap' }}>
              ...
              <h3 style={{ margin: 0, fontSize: '1rem', color: isActive ? 'var(--text-main)' : 'var(--text-muted)', fontWeight: 600 }}>{agent.displayName}</h3>
              <StatusBadge status={isActive ? 'active' : 'inactive'} />
              <span style={{ fontSize: '0.65rem', padding: '0.1rem 0.35rem', background: 'var(--bg-app)', border: '1px solid var(--border)', color: 'var(--text-muted)', borderRadius: '4px', fontWeight: 600, textTransform: 'uppercase' }}>
                {agent.type === 'native' ? 'Native Gemini' : 'Vertex CX'}
              </span>
            </div>
            <p style={{ margin: 0, fontSize: '0.8rem', color: 'var(--text-muted)', lineHeight: 1.4, marginBottom: '0.5rem' }}>
              {agent.description}
            </p>
            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>• {agent.region}</span>
              <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>• {agent.queryType}</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexShrink: 0 }}>
          <button 
            ...
            style={{
              ...
              color: isActive ? 'var(--success)' : 'var(--error)', padding: '0.25rem',
              opacity: saving ? 0.5 : 1, transition: 'color 0.2s'
            }}
          >
            {isActive ? <ToggleRight size={32} strokeWidth={2} /> : <ToggleLeft size={32} strokeWidth={2} />}
          </button>
          <button
        ...
        <div style={{ padding: '1.25rem', background: 'var(--surface)' }}>
          ...
          {agent.tools && agent.tools.length > 0 && (
            <div style={{ marginBottom: '1.5rem' }}>
              <div style={{ fontSize: '0.7rem', fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: '0.5rem' }}>Active Tools</div>
          ...
          <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
            <button
              ...
              style={{
                padding: '0.4rem 0.8rem', borderRadius: '4px', border: '1px solid var(--border)',
                background: 'var(--bg-light)', color: 'var(--text-main)', fontSize: '0.75rem', fontWeight: 600,
                cursor: 'pointer'
              }}
            >
...
function InfoRow({ label, value, mono }) {
  return (
    <div>
      ...
      <div style={{ fontSize: '0.78rem', fontWeight: 600, color: 'var(--text-main)', fontFamily: mono ? 'monospace' : 'inherit', wordBreak: 'break-all' }}>{value}</div>
    </div>
  );
}
    ...
    <div style={{
      ...
      {items.map(({ label, value, color }) => (
        <div key={label} style={{
          ...
          <div style={{ fontSize: '1.6rem', fontWeight: 900, color, letterSpacing: '-0.03em' }}>{value}</div>
          <div style={{ fontSize: '0.72rem', fontWeight: 700, color: 'var(--text-muted)', marginTop: '0.2rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{label}</div>
        </div>
      ))}
    </div>
  );
...
export default function AdminAIAgentsTab() {
  ...
  const handleToggle = async (agentKey, enable) => {
    setSaving(true);
    try {
  ...
  const saveConfig = async () => {
    ...
    try {
      ...
      setAgents(prev => ({
        ...prev,
        [agentKey]: { ...prev[agentKey], model: data.model, systemInstruction: data.systemInstruction },
      }));
  ...
  return (
    <div style={{ maxWidth: '960px', margin: '0 auto', paddingBottom: '4rem' }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          <h2 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 900, color: 'var(--text-main)', letterSpacing: '-0.02em' }}>
            AI Agents
          </h2>
          <p style={{ margin: '0.15rem 0 0', fontSize: '0.8rem', color: 'var(--text-muted)', fontWeight: 500 }}>
        ...
        <button
          ...
          style={{
            ...
            color: 'var(--color-bg-surface)', fontWeight: 700, fontSize: '0.8rem',
      ...
      <div style={{
        ...
        background: 'rgba(59, 130, 246, 0.04)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem'
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          ...
          <div>
            <div style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-main)' }}>Monthly AI Budget Limit: €50.00</div>
            <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Enforced via Google Cloud Billing Alerts.</div>
          </div>
        </div>
        <a 
          ...
          target="_blank" 
          rel="noopener noreferrer"
          style={{ fontSize: '0.8rem', color: '#1a73e8', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.4rem', textDecoration: 'none' }}
        >
      ...
      <div style={{
        ...
        background: 'rgba(59,130,246,0.04)', fontSize: '0.8rem', color: 'var(--text-muted)',
        lineHeight: 1.6,
      }}>
        <strong style={{ color: 'var(--text-main)' }}>🔀 Routing:</strong> Each request's{' '}
        <code style={{ background: 'rgba(0,0,0,0.05)', padding: '0.1rem 0.35rem', borderRadius: '4px' }}>query_type</code>{' '}
        field is matched against the AGENT_REGISTRY in{' '}
        <code style={{ background: 'rgba(0,0,0,0.05)', padding: '0.1rem 0.35rem', borderRadius: '4px' }}>ai_utils.js</code>.{' '}
        Disabling an agent here sets its status in Firestore{' '}
        <code style={{ background: 'rgba(0,0,0,0.05)', padding: '0.1rem 0.35rem', borderRadius: '4px' }}>ai_config/agents</code>,
      ...
      {toast && (
        <div style={{
          ...
          color: 'var(--color-bg-surface)', fontWeight: 700, fontSize: '0.85rem',
      ...
      {editModal.open && (
        <div style={{
          ...
          <div style={{
            ...
            width: '90%', maxWidth: '600px', display: 'flex', flexDirection: 'column', gap: '1.5rem',
            boxShadow: 'var(--shadow-lg)'
          }}>
            <h3 style={{ margin: 0, color: 'var(--text-main)', fontSize: '1.2rem', fontWeight: 800 }}>Edit Configuration: {agents[editModal.agentKey]?.displayName}</h3>
            
            <label style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-muted)' }}>
              Model
              <select
                value={editModal.data.model}
                onChange={e => setEditModal(prev => ({ ...prev, data: { ...prev.data, model: e.target.value } }))}
                style={{
                  padding: '0.75rem', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)',
                  background: 'var(--bg-light)', color: 'var(--text-main)', fontSize: '0.9rem'
                }}
              >
                ...
                <option value="gemini-2.0-flash-lite">gemini-2.0-flash-lite (Ultra Fast, Ultra Cheap)</option>
              </select>
            </label>
            ...
            <label style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-muted)' }}>
              ...
              {agents[editModal.agentKey]?.type === 'vertex' && (
                <div style={{ fontSize: '0.75rem', fontWeight: 500, color: '#f59e0b', background: 'rgba(245,158,11,0.1)', padding: '0.5rem', borderRadius: '4px' }}>
                  Note: This is a Vertex CX agent. Updating instructions here may not override Dialogflow's internal system instructions unless properly synced in Google Cloud. Native Gemini agents will apply these instantly.
                </div>
              )}
              <textarea
                value={editModal.data.systemInstruction}
                onChange={e => setEditModal(prev => ({ ...prev, data: { ...prev.data, systemInstruction: e.target.value } }))}
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem', marginTop: '1rem' }}>
              <button
                ...
                style={{ padding: '0.75rem 1.5rem', borderRadius: 'var(--radius-sm)', border: 'none', background: 'transparent', color: 'var(--text-muted)', fontWeight: 600, cursor: 'pointer' }}
              >
                Cancel
              </button>
              <button
                onClick={saveConfig}
                disabled={saving}
                style={{ padding: '0.75rem 1.5rem', borderRadius: 'var(--radius-sm)', border: 'none', background: 'var(--color-primary)', color: 'white', fontWeight: 600, cursor: saving ? 'not-allowed' : 'pointer' }}
              >
