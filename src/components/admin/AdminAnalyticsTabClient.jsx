  ...
  return (
    <div className="analytics-card">
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem' }}>
        <div
          style={{ fontSize: '1.4rem', fontWeight: 800, color: 'var(--text-main)', lineHeight: 1 }}
        >
          {fmt(currentVal)}
        </div>
...
function getOptimizationTip(label, pct) {
  ...
  const tips = {
    'Content Engagement':
      'Users are leaving without viewing products. Try making the Peptide Catalog more prominent on the Home page.',
    'Add to Cart':
      'Low interest in products. Consider adding more clinical data or high-quality images to peptide descriptions.',
    Checkout:
      'High cart abandonment. Review shipping costs or add a "Quick Checkout" option to reduce friction.',
    Purchase:
      'Checkout friction detected. Ensure payment methods are working and the checkout form is as simple as possible.',
  };
  ...
  return (
    <div
      style={{
        ...
        alignItems: 'center',
        width: '100%',
        marginBottom: index < totalSteps - 1 ? '1rem' : 0,
      }}
    >
      <div
        style={{
          ...
          backgroundImage:
            index === totalSteps - 1
              ? 'linear-gradient(to right, rgba(0,75,135,0.02), transparent)'
              : 'none',
        }}
      >
        <div style={{ flex: 1 }}>
          <div
            style={{
              fontSize: '0.7rem',
              fontWeight: 800,
              color: isBottleneck ? 'var(--error)' : 'var(--text-muted)',
              display: 'flex',
              alignItems: 'center',
              gap: '0.5rem',
        ...
        <div
          style={{
            ...
            background: isBottleneck
              ? 'linear-gradient(to bottom, var(--error), var(--border))'
              : 'var(--border)',
            opacity: 0.4,
            marginTop: '0.5rem',
          }}
...
function TopList({ icon: Icon, title, items, emptyLabel, accentColor = 'var(--secondary)' }) {
  return (
    ...
    >
      <div
        style={{ display: 'flex', alignItems: 'center', gap: '0.8rem', marginBottom: '1.25rem' }}
      >
        <div
          style={{
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', minWidth: 0 }}>
                <div
                  style={{
                    fontSize: '0.65rem',
                    color: i < 3 ? 'white' : 'var(--text-light)',
                    fontWeight: 900,
                    width: '22px',
                    height: '22px',
...
export default function AdminAnalyticsTabClient({ isSubTab, initialData }) {
  ...
  async function load(forceRefresh = false, periodDays = days) {
    ...
    try {
      const res = await fetch(`${ENDPOINT}?days=${periodDays}`, {
  ...
  if (loading) {
    return (
      <div style={{ padding: '5rem 3rem', textAlign: 'center', color: 'var(--text-muted)' }}>
        <div
          style={{ position: 'relative', width: '48px', height: '48px', margin: '0 auto 1.5rem' }}
        >
  ...
  if (error) {
    return (
      ...
      >
        <div
          style={{ backgroundColor: 'rgba(239,68,68,0.1)', padding: '1rem', borderRadius: '50%' }}
        >
          <AlertCircle size={48} color="var(--error)" />
        </div>
        <div>
          <h3 style={{ fontWeight: 900, margin: 0, fontSize: '1.5rem', color: 'var(--text-main)' }}>
        ...
        <button
          ...
          onClick={() => load(true)}
          style={{ padding: '0.8rem 2rem', borderRadius: 'var(--radius-md)' }}
        >
    ...
    <div className="analytics-container">
      ...
      {!isSubTab && (
        <PageHeader
          ...
          subtitle={lastRefresh ? `Synced ${lastRefresh.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} · Analyzing ${days}-day cycle` : 'Real-time analytics and business intelligence'}
          icon={BarChart2}
          actions={
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              {data?.partialData && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.45rem 1rem', borderRadius: '99px', backgroundColor: 'rgba(245,158,11,0.06)', border: '1px solid var(--border)', color: '#b45309', fontSize: '0.75rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                  <AlertCircle size={14} strokeWidth={2.5} />
                  <span>Limited GA4 View</span>
                </div>
              )}
              <div style={{ display: 'flex', backgroundColor: '#f1f5f9', padding: '4px', borderRadius: 'var(--radius-md)', border: '1px solid var(--border)' }}>
                {[7, 30, 90].map((d) => (
                  <button key={d} onClick={() => setDays(d)} style={{ padding: '0.5rem 1.2rem', fontSize: '0.8rem', fontWeight: 900, borderRadius: 'var(--radius-sm)', border: 'none', cursor: 'pointer', backgroundColor: days === d ? 'var(--primary)' : 'transparent', color: days === d ? 'white' : 'var(--text-muted)', transition: 'all 0.2s' }}>
                    {d}D
                  </button>
                ))}
              </div>
              <button style={{ padding: '0', borderRadius: 'var(--radius-md)', height: '40px', width: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'white', border: '2px solid var(--border)', color: 'var(--text-muted)', cursor: 'pointer' }} onClick={() => load(true)}>
      ...
      {/* ── Platform Health ── */}
      {data?.health && <PlatformHealth health={data.health} visitors={metrics.visitors} />}
