...
export default function ClinicalProgressTracker({ protocol }) {
  ...
  const progressData = [
    ...
    { week: 'W4', biomarker: 45, target: 80, phase: 'Initial Response' },
    { week: 'W6', biomarker: 60, target: 80, phase: 'Improvement' },
    { week: 'W8', biomarker: 72, target: 80, phase: 'Improvement' },
    { week: 'W10', biomarker: 85, target: 80, phase: 'Target Achieved' },
    { week: 'W12', biomarker: 90, target: 80, phase: 'Maintenance' },
  ];

  const milestones = [
    { week: 1, label: 'Adaptation', status: 'completed' },
    { week: 3, label: 'Initial Response', status: 'current' },
    { week: 6, label: 'Improvement', status: 'pending' },
    { week: 12, label: 'Target Achieved', status: 'pending' },
  ];
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h3 style={{ margin: '0 0 0.25rem 0', fontSize: '1.1rem', color: '#0f172a', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <Activity size={20} color="#0284c7" /> Expected Clinical Progress
          </h3>
        ...
        {milestones.map((milestone, idx) => (
          <div key={idx} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem', zIndex: 1, width: '25%' }}>
            ...
            <div style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '0.7rem', color: '#64748b' }}>Week {milestone.week}</div>
            </div>
          </div>
        ))}
      </div>
