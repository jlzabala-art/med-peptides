...
import { MailPlus, Send, Mail, CheckCheck, Clock, X, Eye, Trash2, RefreshCw, ArrowUpDown, Search } from 'lucide-react';
    ...
    try {
      await emailjs.send(
        ...
        EMAILJS_TEMPLATE_ID,
        {
          ...
          invite_link: `https://med-peptides-app-27a3a.web.app/?ref=invite&id=${inv.id}`,
          reply_to: 'business@med-peptides.com'
        },
        EMAILJS_PUBLIC_KEY
      );
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '300px', flexDirection: 'column', gap: '1rem' }}>
        ...
        <span style={{ color: 'var(--text-muted)', fontSize: '0.9rem', fontWeight: 650 }}>Loading Invitations...</span>
      </div>
    );
  }
      ...
      <AppDataTable
        ...
        renderBatchActions={(selected) => (
          <button 
            ...
            style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.8rem', color: 'var(--error)', borderColor: 'var(--error)' }}
          >
            ...
            render: (inv) => (
              <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                <AppActionGroup actions={[
                  ...
                  ...(inv.status !== 'accepted' ? [{ type: 'send', onClick: () => handleResend(inv) }] : []),
          ...
          <div style={{ 
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              <div className="show-on-mobile" style={{ display: 'none', gap: '1rem' }}>
                ...
                <span style={{ color: 'var(--text-main)', fontWeight: 500 }}>{inv.status === 'accepted' ? 'Active' : 'Pending'}</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <span style={{ fontWeight: 600, color: 'var(--text-muted)' }}>Date Sent:</span>
                <span style={{ color: 'var(--text-main)', fontWeight: 500 }}>
                  {inv.invitedAt?.toDate ? inv.invitedAt.toDate().toLocaleDateString('en-GB', { day:'2-digit', month:'short', year:'numeric', hour:'2-digit', minute:'2-digit' }) : '—'}
                </span>
              </div>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem' }}>
                <span style={{ fontWeight: 600, color: 'var(--text-muted)' }}>Assigned Roles:</span>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                  {inv.roles && inv.roles.length > 0 ? inv.roles.map(r => (
                    <span key={r} style={{ padding: '2px 6px', backgroundColor: 'var(--color-bg-hover)', color: 'var(--text-main)', borderRadius: '4px', textTransform: 'capitalize', fontWeight: 500 }}>
                      {r.replace('_', ' ')}
                    </span>
              ...
              {inv.message && (
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem' }}>
                  ...
                  <span style={{ fontStyle: 'italic', color: 'var(--text-main)' }}>"{inv.message}"</span>
                </div>
              )}
            </div>
          </div>
      ...
      {previewEmail && (
        <div style={{
          ...
          <div style={{
            ...
            `}</style>
            <div style={{ padding: '1rem 1.5rem', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: 'white' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                <Eye size={20} color="var(--primary)" />
                <h2 style={{ margin: 0, fontSize: '1.1rem', color: 'var(--text-main)' }}>Email Preview: {previewEmail.email}</h2>
              </div>
              <button onClick={() => setPreviewEmail(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)' }}>
                <X size={24} />
              </button>
            </div>
            <div style={{ flex: 1, overflowY: 'auto', backgroundColor: '#f0f4f8', padding: '1rem' }}>
              <div style={{ width: '100%', minHeight: '500px', backgroundColor: 'white', borderRadius: '8px', overflow: 'hidden', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }}>
                <iframe
                  title="Email Preview"
