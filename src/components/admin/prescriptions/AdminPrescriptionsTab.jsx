...
import {
  FileText, Search, Filter, ChevronRight, Calendar, User, Stethoscope, 
  Activity, Clock, Pill, FlaskConical, CheckCircle2, AlertCircle, Eye,
  Archive, FilePlus, Package
} from 'lucide-react';
...
function UniformKPIs({ data }) {
  ...
  const awaiting  = data.filter(d => ['assigned_to_wholesaler', 'draft'].includes(d.status)).length;
  const active    = data.filter(d => ['Active', 'active', 'sent', 'viewed_by_patient', 'ordered', 'added_to_bulk'].includes(d.status)).length;
  ...
  const stats = [
    ...
    { label: 'Expiring Soon (< 14d)', value: expiringSoon, color: '#ef4444', icon: <AlertCircle size={16} /> },
  ];
              ...
              <tr key={rx.id} className="flexible-row" onClick={() => onRowClick(rx)}>
                <td data-label="Patient & Doctor">
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    ...
                    <div>
                      <div style={{ fontWeight: 700, color: '#0f172a', fontSize: '0.95rem' }}>{patient}</div>
                      <div style={{ fontSize: '0.8rem', color: '#64748b', display: 'flex', alignItems: 'center', gap: '0.2rem' }}>
                ...
                <td data-label="Protocol">
                  <div style={{ fontWeight: 600, color: '#334155', fontSize: '0.9rem' }}>{protocol}</div>
                  <div style={{ fontSize: '0.8rem', color: '#64748b' }}>{diagnosis}</div>
                </td>
                
                <td data-label="Source & APIs">
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.85rem', color: '#334155', fontWeight: 600 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.8rem', color: '#64748b', marginTop: '0.2rem' }}>
                    <FlaskConical size={12} color="#8b5cf6" /> {apiCount} item{apiCount !== 1 ? 's' : ''}
                  </div>
                </td>
...
export default function AdminPrescriptionsTab() {
  ...
  const filtered = prescriptions.filter(p => {
    // 1. Search text
    const patient  = (p.patient?.name || p.patientName || '').toLowerCase();
    const doctor   = (p.doctor?.name || p.doctorName || '').toLowerCase();
    const id       = (p.id || '').toLowerCase();
    const q        = searchTerm.toLowerCase();
    const matchesSearch = !searchTerm || patient.includes(q) || doctor.includes(q) || id.includes(q);
    
    // 2. Smart Chip filtering
    let matchesChip = true;
    if (activeChip === 'awaiting') {
      matchesChip = ['assigned_to_wholesaler', 'draft'].includes(p.status);
    } else if (activeChip === 'active') {
      matchesChip = ['Active', 'active', 'sent', 'viewed_by_patient', 'ordered', 'added_to_bulk'].includes(p.status);
    } else if (activeChip === 'recent') {
      if (p.createdAt?.toDate) {
        const diffDays = (new Date() - p.createdAt.toDate()) / (1000 * 60 * 60 * 24);
        matchesChip = diffDays <= 7;
      } else {
        matchesChip = false;
      }
    }
  ...
  return (
    <>
      ...
      <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', height: '100%', gap: '1.25rem', backgroundColor: '#f8fafc' }}>
        
        {/* Page Header */}
        <div>
          <h1 style={{ fontSize: '1.8rem', fontWeight: 800, color: '#0f172a', margin: '0 0 0.2rem 0', display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', alignItems: 'center', justifyContent: 'space-between', background: 'white', padding: '1rem', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {loading ? (
            <div style={{ padding: '2rem', textAlign: 'center', color: '#94a3b8' }}>Loading prescriptions...</div>
          ) : filtered.length === 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '5rem 2rem', gap: '1rem', color: '#94a3b8', background: 'white', borderRadius: '12px', border: '1px dashed #cbd5e1' }}>
              ...
              <div style={{ textAlign: 'center' }}>
                <h3 style={{ margin: '0 0 0.25rem 0', color: '#475569', fontWeight: 700 }}>No Prescriptions Found</h3>
                <p style={{ margin: 0, fontSize: '0.85rem' }}>
                  {searchTerm || activeChip !== 'all' ? 'Try adjusting your search or filters.' : 'Prescriptions will appear here once created.'}
                </p>
              </div>
            </div>
          ) : (
        ...
        {/* Detail Modal */}
        {selectedItem && (
          <PrescriptionDetailModal rx={selectedItem} onClose={() => setSelectedItem(null)} />
        )}
      </div>
    </>
  );
