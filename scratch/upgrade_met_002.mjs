...
async function upgradeMet002() {
  const protocolId = 'met_002';
  ...
  updatePayload.variant_rules = {
    ...
    sex_variants: {
      female: { special_notes: ["Monitor cycle-related metabolic shifts and appetite fluctuations."] },
  ...
  // 3. Map phases to phase_blueprints if missing
  if (!data.phase_blueprints && data.phases) {
    updatePayload.phase_blueprints = data.phases.map(p => ({
      ...
      drugs: p.drugs_used.map(d => ({
        ...
        route: d.route === 'subcutaneous' ? 'SC' : 'SC',
        dose_logic: {
          dose_unit: "mg",
...
upgradeMet002().catch(console.error);
