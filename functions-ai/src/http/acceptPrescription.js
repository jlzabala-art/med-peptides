...
exports.acceptPrescription = onCall(async (request) => {
  // Authentication checking can be added if needed
  // if (!request.auth) throw new HttpsError('unauthenticated', 'Must be logged in');

  const { queueItemId, patientName, patientDOB, doctorName, products } = request.data;
  if (!queueItemId) throw new HttpsError('invalid-argument', 'Missing queueItemId');
