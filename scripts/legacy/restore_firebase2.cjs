...
for (const file of files) {
    ...
    if (match) {
        ...
        // Find assignments like const auth = fb?.auth; or const auth=fb?.auth;
        ...
        if (assignments.length > 0) {
            ...
            // No fb?.auth assignments found? Maybe just remove the import if unused.
        }
    }
    
    if (content !== original) {
        fs.writeFileSync(file, content);
        console.log("Restored double quotes:", file);
    }
}
