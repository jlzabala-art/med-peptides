const fs = require('fs');
const file = 'src/hooks/shared/usePrescriptionAI.js';
let content = fs.readFileSync(file, 'utf8');

const replacement = `export function usePrescriptionAI() {
  const [isProcessing, setIsProcessing] = useState(false);
  const [error, setError] = useState(null);

  const queuePrescription = async (file, currentUid = 'anonymous') => {
    // 1. Upload to Storage
  ...
  const processPrescription = useCallback((file, currentUid = 'anonymous') => {
    return new Promise(async (resolve, reject) => {
      setIsProcessing(true);
      setError(null);

      try {
        const { docId, docRef, downloadURL } = await queuePrescription(file, currentUid);
...
content = content.replace(/export function usePrescriptionAI\(\) \{[\s\S]*\}\n$/, replacement);
fs.writeFileSync(file, content);
console.log("usePrescriptionAI updated.");
